...
`rescue-services.js` parses approved rows and uses `RescueDaily.standings` for Daily, Weekly, All time and Daily winners. Published form timestamps are normalized to a safe ISO timestamp on the correct sheet date; form row order preserves earlier-score tie priority.
