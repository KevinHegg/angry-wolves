...
Players use `https://kevinhegg.github.io/angry-wolves/`. Release **2.49** keeps the Restart game label, dims and disables it after three Free play restarts, and restores it upon entering the orchard. The masthead wolf has a small CSS-only red-eye pulse; reduced-motion preferences leave the eyes visible without animation. It retains the v2.48 Daily leaderboard date navigation and five visible score rows. Official winner finalization remains inactive pending Google authorization. The root loads immutable assets from `play/2.49/`; the version appears only on the opening screen. All historical `play/2.x/` entry pages and `classic.html` redirect to the main URL. The temporary refresh query bypasses old cached redirects and is removed from the address bar.
