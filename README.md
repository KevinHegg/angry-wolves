...
Players use `https://kevinhegg.github.io/angry-wolves/`. The current release is **2.43**. The published root loads immutable assets from `play/2.43/`; the version appears only on the opening screen. All historical `play/2.x/` entry pages and `classic.html` redirect to the main URL. The temporary refresh query bypasses old cached redirects and is removed from the address bar.
