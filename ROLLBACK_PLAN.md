...
## v2.41 — Completed daily challenge and explicit wolf pressure

Baseline: v2.40 commit `86d68b4`. Winning a daily challenge now makes the main Play free action start an ordinary random adventure instead of silently replaying the same daily seed. The opening panel marks today complete and offers the saved daily result for posting; daily replay remains a deliberate choice. A separate `hw-daily-completed` record preserves the latest completed date and personal best through free play and an abandoned replay. Completed v2.40 saves migrate, including a reload during the ending animation. Safari back-cache refreshes the chooser, while active games keep their state. Completion is local to the browser; cleared site data cannot be restored from the device, and blocked storage retains the marker only during the page session.

The trail now says which whistle is next and how many steps a small herd moves the wolf. Before whistles 18 and 26, an announcement explains all three herd-size effects; selected herds also name their upcoming whistle and exact movement. Existing message space is reused so warnings do not resize the board. Help and introductory rules distinguish the first 17 whistles from later pressure. Game rules, daily seeds, score values and the score service are unchanged.
...
## v2.46 — consolidated live leaderboards
...
## v2.46 — wolf pressure, final gate, and Pip bonus tuning
...
Validation at release: 180 Node tests passed; the static build succeeded; `git diff --check` was clean. A browser visual check showed the red eyes at the actual wolf trail size. Seeded balance checks completed and are tuning probes, not estimates of human win rates. v2.46 was deployed without a production score submission. The official Apps Script results backend remains inactive.
...
## v2.51 — simpler late wolf pressure

The baseline wolf rule is unchanged for whistles 1–14. Whistles 15–29 move the wolf two steps closer for herds of 3–5, while 6+ hold it still. From whistle 30, every herd moves it two steps closer. No late herd size has a one-step approach, and large herds cannot extend play indefinitely. `wolfMove()` in `rescue-engine.js` owns the three tables; selection previews, warnings and help copy read the same thresholds. To roll back, restore v2.50 `rescue-engine.js` movement and the matching copy in `rescue.js`, `index.html` and `README.md`, then repackage under a new version. The previous production commit is tagged `checkpoint/pre-simpler-pressure-2.50`; its complete assets remain in `play/2.50/`. This release occurred during an open daily challenge at the user's request; only one score had been posted, and existing scores remain unchanged.

## v2.52 — share text, desktop image tools and link previews

Game rules, scoring and the submission contract are unchanged. Native shares now pass the score caption (without its link) as `text` beside the card and `url`; this replaces the 2.2 decision to send no caption. Copy score & link copies the full caption. Where `navigator.share` cannot take files, the results panel shows Copy image (`navigator.clipboard.write` with a `ClipboardItem` built synchronously in the click) and Save image (a download of the prepared PNG). The card prints its address derived from `GAME_URL`. `index.html` carries Open Graph and Twitter card tags pointing at the static `hungry-wolf-card.png`, a score-free card drawn by `makeCard({poster:true})` through the unpublished `scripts/card-poster.html`; `scripts/build-site.cjs` publishes that image. The dialog body also hides horizontal overflow and gains 2px of side padding, because the heading's fence decoration extended 2px past the body and drew a phantom horizontal scrollbar in Safari on every panel. To roll back, restore v2.51 `rescue-share.js`, `rescue.js`, `rescue-services.js`, `rescue.css` and `index.html`, drop the image from the site builder, and package under a new version. The previous production commit is tagged `checkpoint/pre-share-social-2.51`; its complete assets remain in `play/2.51/`.
