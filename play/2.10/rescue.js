(() => {
  ...
  let fieldEntryBoard = null,fieldEntryCats={};
  ...
  function render() {
    ...
    const imminent=R.burstPreview(state),catCount=Object.keys(state.cats).length;
    $('cat-warning').hidden=!catCount;
    $('cat-warning').textContent=imminent.cats.length?`${imminent.cats.length} ${imminent.cats.length===1?'cat bursts':'cats burst'} next · +${imminent.cats.length} wolf ${imminent.cats.length===1?'step':'steps'}${state.bark?' · Pip stops them!':''}`:`${catCount} ${catCount===1?'magic cat':'magic cats'} · smile builds a herd · scowl bursts`;
    const blastArea=new Set(imminent.animals);
    const watchedArea=new Set(R.catNeighbors(state));
    const playable = new Set(R.groups(state.board).flat());
    $('board').innerHTML = state.board.map((t,i)=>`<button class="animal ${watchedArea.has(i)?'cat-watched':''} ${blastArea.has(i)?'blast-warning':''} ${R.isCat(t)?`magic-cat ${t===R.GOOD_CAT?'good-cat':'evil-cat'}`:''} ${playable.has(i)?'hint':''}" data-cell="${i}" data-type="${t}" aria-label="${R.isCat(t)?`${t===R.GOOD_CAT?'Smiling cat, makes a herd':'Evil cat, bursts'} after ${state.cats[i]} ${state.cats[i]===1?'rescue':'rescues'}. Pip can scare it away`:singular[t]}, row ${Math.floor(i/6)+1}, column ${i%6+1}" aria-pressed="false" tabindex="${i===30?0:-1}">${icon(R.isCat(t)?(t===R.GOOD_CAT?6:5):t)}${R.isCat(t)?`<span class="cat-life" aria-hidden="true">${Array.from({length:3},(_,dot)=>`<i class="${dot<state.cats[i]?'lit':''}"></i>`).join('')}</span>`:''}</button>`).join('');
    $('bark').disabled = !state.bark || busy || state.status!=='playing';
    $('bark').innerHTML = `<span aria-hidden="true">🐕</span><span>${state.bark?'Pip, bark!':'Good dog, Pip.'}<small>${state.bark?(Object.keys(state.cats).length?'+3 steps · shoo all cats':'+3 steps · regroup · once'):'Bark used this field'}</small></span>`;
  ...
  function select(i) {
    ...
    if(R.isCat(state.board[i])){selected=[];renderSelection();say(`${state.board[i]===R.GOOD_CAT?'A smiling cat! Matches two tiles for the largest herd in':'An evil cat! Bursts in'} ${state.cats[i]} ${state.cats[i]===1?'rescue':'rescues'}. Each lit dot is one rescue. Pip can quietly shoo all cats away.`);return;}
  ...
  function commit() {
    ...
    setTimeout(()=>{
      ...
      say(`${result.count} ${names[result.type]} home! ${result.step>0?'The pack backs away.':result.step===0?'The pack holds back.':`The pack moves ${-result.step} ${result.step===-1?'step':'steps'} closer.`}${result.catAppeared>=0?` Poof! A ${state.board[result.catAppeared]===R.GOOD_CAT?'smiling':'evil'} cat appeared for ${state.cats[result.catAppeared]} rescues.`:result.burstCats.length?` Poof! ${result.burstCats.length===1?'A cat burst':`${result.burstCats.length} cat bursts`} rescued ${result.burstAnimals.length} extra animals, no extra points.`:''}${result.gifts.length?` A smiling cat made a herd of ${result.gifts[0].size} ${names[result.gifts[0].type]}! Whistle it home for points.`:''}${result.regrouped?' A new herd gathered.':''}`);
      if(state.status!=='playing')finishField();
    },reducedMotion.matches?0:230);
  }
  function startField(chapter, carriedBoard = null,cats={}) {
    generation++;busy=false;selected=[];ready=true;
    state=R.create(chapter,Math.random,carriedBoard,cats);fieldEntryBoard=state.board.slice();fieldEntryCats={...state.cats};render();window.scrollTo(0,0);
  ...
  function finishField() {
    ...
    openDialog({eyebrow:`FIELD ${state.chapter+1} OF 3 COMPLETE`,title:['The sheep are safe.','Out of the orchard.'][state.chapter],copy:`${R.CHAPTERS[state.chapter].ending} ${next.story}`,
      details:`<div class="stat-line"><span><strong>${total+state.saved.reduce((a,b)=>a+b,0)}</strong>animals home</span><span><strong>${rested}</strong>fields Pip rested</span></div><p class="dialog-tip">Next: ${next.name}. Your animals carry forward. Pip gets a fresh bark. From field two, one black cat may visit at a time, with a 2–3 rescue timer. Smiling cats build herds; evil cats burst and attract wolves. Pip can dismiss all cats quietly.</p>`,
      action:`Continue to field ${state.chapter+2}`,run:()=>{
        ...
        const chapter=state.chapter+1,board=state.board.slice(),cats={...state.cats};closeDialog();tone('gate');startField(chapter,board,cats);
      }});
  }
  ...
  function showLeaderboard() {
    ...
    openDialog({view:'leaderboard',eyebrow:'THE SHEPHERDS’ BOARD',title:'A place among the herd.',copy:'The highest 20 adventures stay on the board. Beat the lowest score to take a place; earlier scores win ties.',
  ...
  $('bark').addEventListener('click',()=>{
    ...
    const finish=()=>{
      ...
      say(catIndices.length?'Woof! All cats vanished quietly. Animals tumble into the gaps.':'Good dog! Wolves back 3 steps. The animals regrouped.');
    };
