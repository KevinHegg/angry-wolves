(() => {
  ...
  function play(kind = 'select', options = {}) {
    ...
    return pending.then(audio => {
      ...
      const voice=(kind==='select'||kind==='rescue')?voices.ANIMALS[options.animal]:['bark','snarl','whimper','whoosh'].includes(kind)?kind:null;
      if(voice){
        const lane=kind==='snarl'||kind==='whimper'?'wolf':kind==='whoosh'?'wind':'herd',short=kind==='select';
