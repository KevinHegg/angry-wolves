...
(function (root) {
  ...
  const WIND_SETTINGS = Object.freeze({enabled:true,maxCats:1,firstChapter:1,chance:.25,minMoves:2,minLife:3,maxLife:3,cooldown:1,kind:"wind"});
  ...
  function create(chapter = 0, rng = Math.random, carriedBoard = null, carriedCats = {}, settings = WIND_SETTINGS) {
    ...
    return { chapter, board, catSettings:settings, cats:Object.fromEntries(board.flatMap((t,i)=>isSpecial(t)?[[i,Math.min(3,Math.max(1,carriedCats[i]||3))]]:[])),catCooldown:0, saved: [0, 0, 0, 0], distance: c.distance, bark: 1, moves: 0, score: 0, biggest: {count: 0, type: 0}, status: 'playing' };
  }
  function rescue(state, start, rng = Math.random) {
    ...
    for(const i of Object.keys(state.cats).map(Number))if(state.board[i]===DUST){
      const moves=rotateWind(state.board,i);rotations.push({index:i,moves,completed:3-state.cats[i]});
      if(state.cats[i]===0){
  ...
  // Compass order stays clockwise even when an edge omits a position.
  function windRing(board,index){
    return [index>=SIZE?index-SIZE:-1,index%SIZE<SIZE-1?index+1:-1,index<SIZE*(SIZE-1)?index+SIZE:-1,index%SIZE?index-1:-1].filter(i=>i>=0&&!isSpecial(board[i]));
  }
  function rotateWind(board,index){
    const ring=windRing(board,index),before=ring.map(n=>board[n]);
    const moves=ring.map((from,j)=>({from,to:ring[(j+1)%ring.length],type:before[j]}));
  ...
  const api = { SIZE, CAT, GOOD_CAT, DUST, isCat, isSpecial, WIND_SETTINGS, windRing, rotateWind, windGift, bestGift, catNeighbors, CAT_SETTINGS, CHAPTERS, REST_BONUSES, restBonus, wolfStep, herdPoints, pressure, neighbors, group, groups, create, rescue, bark, burstPreview };
