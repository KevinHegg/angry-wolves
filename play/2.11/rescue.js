(() => {
  ...
  let fieldEntryBoard = null,fieldEntryCats={},fieldEntryDistance=state.distance;
  ...
  function render() {
    ...
    $('cat-warning').textContent=windIndices.length?`↻ Clockwise after each rescue · ${3-state.cats[windIndices[0]]}/3 turns filled`:'';
  ...
  function select(i) {
    ...
    if(R.isSpecial(state.board[i])){selected=[];renderSelection();say(`A dust devil! ${3-state.cats[i]}/3 turns complete. Neighbors move clockwise after each rescue. Then the gust becomes the animal making the largest herd.`);return;}
  ...
  function animateWind(rotations){
    ...
    for(const rotation of rotations){
      for(const move of rotation.moves){
        const svg=cells[move.to]?.querySelector?.('svg');if(!svg)continue;
        const from=cells[move.from].getBoundingClientRect(),to=cells[move.to].getBoundingClientRect();
        svg.animate([{transform:`translate(${from.left-to.left}px,${from.top-to.top}px)`,opacity:.65},{transform:'translate(0,0)',opacity:1}],{duration:450,easing:'ease-in-out'});
      }
      cells[rotation.index]?.classList.add('wind-turn');
      if(rotation.completed===3)cells[rotation.index]?.classList.add('wind-finished');
    }
  }
  function startField(chapter, carriedBoard = null,cats={},distance=R.CHAPTERS[chapter].distance) {
    generation++;busy=false;selected=[];ready=true;
    state=R.create(chapter,Math.random,carriedBoard,cats);state.distance=distance;fieldEntryDistance=distance;fieldEntryBoard=state.board.slice();fieldEntryCats={...state.cats};render();window.scrollTo(0,0);
  ...
  function finishField() {
    ...
    openDialog({eyebrow:`FIELD ${state.chapter+1} OF 3 COMPLETE`,title:['The sheep are safe.','Out of the orchard.'][state.chapter],copy:`${R.CHAPTERS[state.chapter].ending} ${next.story}`,
      details:`<div class="stat-line"><span><strong>${total+state.saved.reduce((a,b)=>a+b,0)}</strong>animals home</span><span><strong>${rested}</strong>fields Pip rested</span></div><p class="dialog-tip">Next: ${next.name}. Your animals carry forward. The wolves stay ${state.distance} steps away. Pip gets a fresh bark. From field two, a dust devil can turn neighboring animals clockwise for three rescues. Then it settles into an animal. Pip can lead the animals clear.</p>`,
      action:`Continue to field ${state.chapter+2}`,run:()=>{
        ...
        const chapter=state.chapter+1,board=state.board.slice(),cats={...state.cats},distance=state.distance;closeDialog();tone('gate');startField(chapter,board,cats,distance);
      }});
  }
