(() => {
  ...
  function intro() {
    ...
    openDialog({eyebrow:'A SMALL ADVENTURE IN THREE CHAPTERS',title:'The gate was left open.',
      ...
      details:'<div class="instruction"><b>1.</b> Select 3+ matching animals touching side to side.</div><div class="instruction"><b>2.</b> Whistle them home. A herd of 3 brings wolves two steps closer; 5+ holds them off.</div><div class="instruction"><b>3.</b> Fill the animal goals. Let Pip rest for a bonus at the end.</div><p class="dialog-tip">No clock. The wolves wait for your move. 🔊 Use the speaker button to turn sound on or off.</p><p class="intro-version">v'+S.VERSION.replace('rescue-','')+'</p>',
  ...
  function commit() {
    ...
    setTimeout(()=>{
      ...
      if(busy)setTimeout(()=>{if(ticket!==generation)return;busy=false;render();},1500);
      ...
      if(state.status!=='playing'){
        ...
        setTimeout(()=>{if(ticket===generation)finishField();},reducedMotion.matches?350:result.rotations.length?1700:900);
      }
  ...
  function animateWind(rotations){
    ...
    for(const rotation of rotations){
      ...
      for(const [j,move] of rotation.moves.entries()){
        ...
        const positions=[move.from,destinations[(j+1)%destinations.length],destinations[(j+2)%destinations.length],move.to];
        ...
        // Two temporary arrangements, with a pause at each, then the real result.
        const [start,first,second,end]=positions.map(frame);
        svg.animate([{...start,offset:0},{...first,offset:.23},{...first,offset:.34},{...second,offset:.57},{...second,offset:.68},{...end,offset:1}],{duration:1400,easing:'ease-in-out'});
      }
