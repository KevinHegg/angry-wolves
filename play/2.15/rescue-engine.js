...
(function (root) {
  ...
  const CHAPTERS = [
    ...
    { name: 'Trouble in the orchard', time: 'Sunset', goal: [0, 16, 16, 0], types: 3, distance: 6,
      story: 'The pigs and hens heard the howling. They are hiding in the orchard. Bring 16 of each back to the lane.',
      ending: 'The orchard is quiet. One last herd is stranded beside the woods.' },
    { name: 'The last gate', time: 'Nightfall', goal: [12, 12, 12, 12], types: 4, distance: 5,
      story: 'The barn lantern is lit. The pack is close. Cows will join as you make space. Bring 12 of every animal home, then shut the last gate.',
  ...
  function create(chapter = 0, rng = Math.random, carriedBoard = null, carriedCats = {}, settings = WIND_SETTINGS, carriedWait = null) {
    ...
    return { chapter, board, windWait:carriedWait??windWait(rng), catSettings:settings, cats:Object.fromEntries(board.flatMap((t,i)=>isSpecial(t)?[[i,Math.min(3,Math.max(1,carriedCats[i]||3))]]:[])),catCooldown:0, saved: [0, 0, 0, 0], distance: c.distance, bark: 1, moves: 0, score: 0, biggest: {count: 0, type: 0}, status: 'playing' };
  }
