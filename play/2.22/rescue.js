(() => {
  ...
  function render() {
    ...
    $('bark').innerHTML = `<span aria-hidden="true">🐕</span><span>${state.bark?'Pip, bark!':'Good dog, Pip.'}<small>Send wolf back 3 steps</small><small class="pip-charge"><i class="${state.bark?'':'filled'}" aria-hidden="true"></i>${state.bark?'1 bark this field':'Bark used this field'}</small></span>`;
