(() => {
  ...
  function showLeaderboard() {
    ...
    openDialog({view:'leaderboard',eyebrow:'THE SHEPHERDS’ BOARD',title:'The top shepherds',copy:'Top 20 adventures. Beat the lowest score to join; earlier scores win ties.',
      details:`${form?`<details class="score-entry" open><summary>Your player · ${finalResult.score.toLocaleString()} points</summary>${form}</details>`:''}<p id="submit-status" class="dialog-tip" role="status"></p><p id="leaderboard-status" role="status">Loading scores…</p><ol id="leaderboard-list" class="leaderboard-list" tabindex="0" aria-label="Top scores. Scroll for more."></ol><button id="refresh-scores" class="text-button" type="button">Refresh scores</button>`,
  ...
  async function postScore(event){
    ...
    try{
      ...
      if(dialogView==='leaderboard'){$('submit-score').hidden=true;$('score-form')?.remove();$('submit-status').textContent=submission.message;loadLeaderboard();}
