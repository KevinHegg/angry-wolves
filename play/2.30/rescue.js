(() => {
  ...
  function commit() {
    ...
    setTimeout(()=>{
      ...
      say(`${result.count} ${names[result.type]} home! ${result.step>0?'The pack backs away.':result.step===0?'The pack holds back.':`The pack moves ${-result.step} ${result.step===-1?'step':'steps'} closer.`}${result.catAppeared>=0?' A dust devil rolls in!':''}${windRotations.length?' Whoosh! Nearby animals scatter.':''}${!gameOver&&result.gifts.length?` The wind settles into ${names[result.gifts[0].type]}, making a herd of ${result.gifts[0].size}.`:''}${result.regrouped?' A new herd gathered.':''}`);
      if(state.status!=='playing'){
        if(gameOver){
          ...
          setTimeout(()=>{if(ticket!==generation)return;busy=false;finishField(true);},2600);
  ...
  $('bark').addEventListener('click',()=>{
    ...
    const finish=()=>{
      ...
      say(Object.keys(state.cats).length?'Good dog! Wolves back 3 steps. Animals regroup; the gust and its countdown stay put.':'Good dog! Wolves back 3 steps. The animals regrouped.');
    };
