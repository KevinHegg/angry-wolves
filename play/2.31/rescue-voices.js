...
(function(root){
  ...
  const VOICES={
    ...
    'howl-plaintive':{pitch:360,end:185,length:2.3,vibrato:4,wobble:.022,rough:.002,formants:[550,1050]},
    'howl-deep':{pitch:145,end:110,length:2.5,vibrato:3,wobble:.025,rough:.012,formants:[330,750]},
  ...
  function synthesize(kind,short=false,sampleRate=22050){
    if(kind==='howl-plaintive'||kind==='howl-deep'){
      const sad=kind==='howl-plaintive',duration=short?1:(sad?2.05:2.1);
      ...
      for(let i=0;i<samples.length;i++){
        ...
        const pitch=(sad?220+120*rise-175*fall:100+55*rise-30*fall)*(1+(sad?.012:.006)*Math.sin(2*Math.PI*4.5*t));
        ...
        for(let h=1;h<=14;h++){
          ...
          voice+=Math.sin(h*phase)*(.2+shape)/(h**1.1);
        }
        const envelope=Math.sin(Math.PI*u)**(sad?1.15:.75)*Math.min(1,t/.14);
        samples[i]=Math.tanh(voice*.7)*envelope*(sad?.29:.33);
      }
      return samples;
    }
    if(kind==='whoosh'){
      ...
      for(let i=0;i<samples.length;i++){
        ...
        samples[i]=Math.tanh((smooth-rumble)*3)*swell**1.6*(.45+.55*gust)*.4;
      }
      return samples;
    }
    if(kind==='pig'){
    ...
    for(let i=0;i<samples.length;i++){
      const t=i/sampleRate,u=t/duration;
      const howl=kind.startsWith('howl-');
      const contour=howl?Math.min(1,u/.2):u;
      const base=howl?(v.pitch*.65+v.pitch*.35*contour+(v.end-v.pitch)*Math.max(0,(u-.3)/.7)):(v.pitch+(v.end-v.pitch)*u);
      ...
      }else envelope*=Math.sin(Math.PI*u)**(howl?1.1:.45);
