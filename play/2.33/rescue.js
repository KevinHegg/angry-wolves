(() => {
  ...
  function intro() {
    ...
    openDialog({view:'intro',eyebrow:'A SMALL ADVENTURE IN THREE CHAPTERS',title:'The gate was left open.',
      ...
      details:'<div class="instruction"><b>1.</b> Select 3+ matching animals touching side to side.</div><div class="instruction"><b>2.</b> Whistle them home. Herds of 3 or 4 bring the wolf one step closer; 5–6 hold them still; 7+ push it back.</div><div class="instruction"><b>3.</b> Fill the animal goals. Pip has 3 barks for the whole game: each sends the wolf back 3 steps and regroups animals. Open dots are available; filled dots are used.</div><p class="dialog-tip">Win with Pip resting in 1 / 2 / 3 fields for +100 / +300 / +650 points. No clock—the wolf waits for your move. 🔊 Use the speaker button to turn sound on or off.</p>',
      action:'Let’s bring them home', run:() => {ready=true; startedAt=Date.now(); tone('gate'); closeDialog(); selected=[];renderSelection();say('Select a herd of 3+ matching animals that touch.');} });
  }
  function render() {
    const c = R.CHAPTERS[state.chapter];
    $('retry').textContent='Restart game';
    ...
    $('wolf-effect').textContent = R.pressure(state.moves+1)?`Long field: the wolf advances +${R.pressure(state.moves+1)} extra per whistle`:`Whistle ${state.moves+1} · extra wolf pressure from 18`;
  ...
  function renderSelection() {
    ...
    $('herd-hint').textContent=selected.length?'Tap Whistle to bring them home.':'Tap 3+ matching animals.';
  }
  function select(i) {
    ...
    const effect=step>0?'The wolf steps back!':step===0?'The wolf stays put.':`The wolf ${-step} ${step===-1?'step':'steps'} closer${state.distance<=-step?' — finish the goal!':'.'}`;
  ...
  function commit() {
    ...
    setTimeout(()=>{
      ...
      selected=[];busy=!!windRotations?.length&&!reducedMotion.matches;render();
      if(!result.ok)return;
      const trail=$('trail-steps').children,from=trail[previousDistance],to=trail[state.distance];
      const wolf=to?.querySelector?.('svg');
      if(!gameOver&&wolf&&!reducedMotion.matches&&previousDistance!==state.distance){
      ...
      say(`${result.count} ${names[result.type]} home! ${result.step>0?'The wolf backs away.':result.step===0?'The wolf holds back.':`The wolf moves ${-result.step} ${result.step===-1?'step':'steps'} closer.`}${result.catAppeared>=0?' A dust devil rolls in!':''}${windRotations.length?' Whoosh! Nearby animals scatter.':''}${!gameOver&&result.gifts.length?` The wind settles into ${names[result.gifts[0].type]}, making a herd of ${result.gifts[0].size}.`:''}${result.regrouped?' A new herd gathered.':''}`);
      if(state.status!=='playing'){
        if(gameOver){
          ...
          setTimeout(()=>{
            ...
            setTimeout(()=>{if(ticket!==generation)return;busy=false;finishField(true);},2000);
          },0);
        }else setTimeout(()=>{if(ticket===generation)finishField();},reducedMotion.matches?350:windRotations.length?2700:900);
      }
  ...
  function startField(chapter, carriedBoard = null,cats={},distance=R.CHAPTERS[chapter].distance,wait=null,barks=3) {
    ...
    state=R.create(chapter,Math.random,carriedBoard,cats,undefined,wait,barks);fieldEntryWait=state.windWait;state.distance=distance;fieldEntryDistance=distance;fieldEntryBoard=state.board.slice();fieldEntryCats={...state.cats};render();window.scrollTo(0,0);
  ...
  function finishField(endingPresented=false) {
    ...
    openDialog({eyebrow:`FIELD ${state.chapter+1} OF 3 COMPLETE`,title:['The sheep are safe.','Out of the orchard.'][state.chapter],copy:`${R.CHAPTERS[state.chapter].ending} ${next.story}`,
      details:`<div class="stat-line"><span><strong>${total+state.saved.reduce((a,b)=>a+b,0)}</strong>animals home</span><span><strong>${3-state.bark} of 3</strong>Pips used · ${state.bark?`${state.bark} left`:"no Pips left"}</span></div><p class="dialog-tip">Next: ${next.name}. Your animals carry forward. The wolf stays ${state.distance} steps away. Pip has ${state.bark} bark${state.bark===1?'':'s'} left for the game. Dust devils wait three rescues, scatter surrounding animals, then settle into an animal. Pip leaves the gust and its countdown unchanged.</p>`,
  ...
  function showLeaderboard() {
    ...
    const form=finalResult&&!submission.done?`<form id="score-form"><p id="player-lock-status" class="dialog-tip">${locked?'Saved player locked. Your previous scores keep their original name.':'Choose a player for this and future scores. Previous scores keep their original name.'}</p><fieldset id="player-fields" class="player-fields" ${locked?'disabled':''}><span class="initials-label">Your three letters</span><input id="score-initials" type="hidden"><div class="letter-wheels" role="group" aria-label="Three-letter player name">${[0,1,2].map(i=>`<div class="letter-wheel"><button type="button" data-letter="${i}" data-step="1" aria-label="Next letter ${i+1}">▲</button><output id="player-letter-${i}" aria-live="polite">${(initials||'AAA')[i]}</output><button type="button" data-letter="${i}" data-step="-1" aria-label="Previous letter ${i+1}">▼</button></div>`).join('')}</div><fieldset class="badge-picker"><legend>Choose your shepherd badge</legend>${badgeChoices.map(i=>`<label><input type="radio" name="shepherd-badge" value="${i}" ${i===badge?'checked':''}><span title="${S.BADGE_NAMES[i]}" aria-label="${S.BADGE_NAMES[i]}">${S.BADGES[i]}</span></label>`).join('')}</fieldset></fieldset></form>`:'';
    ...
    if($('score-initials')){
      ...
      $('toggle-player-lock').onclick=()=>{
        if($('player-fields').disabled){
          profileUnlocked=true;$('player-fields').disabled=false;$('toggle-player-lock').textContent='Save player';
          ...
          profileUnlocked=false;rememberProfile();$('submit-status').textContent='Player saved. You can now post your score.';$('player-fields').disabled=true;$('toggle-player-lock').textContent='Change player';
  ...
  async function loadLeaderboard(){
    ...
    try{
      ...
      if(finalResult && !submission.done && $('submit-score')) {
        ...
        $('submit-status').textContent=eligible ? submission.message || 'Your score qualifies. Post it with the player shown above.' : `This time you need more than ${entries[S.LIMIT-1].score.toLocaleString()} points to enter the top 20.`;
      }
  ...
  $('bark').addEventListener('click',()=>{
    ...
    const finish=()=>{
      ...
      say(Object.keys(state.cats).length?'Good dog! Wolf back 3 steps. Animals regroup; the gust and its countdown stay put.':'Good dog! Wolf back 3 steps. The animals regrouped.');
    };
  ...
  $('retry').addEventListener('click',freshAdventure);
