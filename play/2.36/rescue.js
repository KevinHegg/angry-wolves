(() => {
    ...
    if($('score-initials')){
      ...
      $('toggle-player-lock').onclick=()=>{
        ...
        if($('player-fields').disabled){
          ...
          $('player-summary').textContent=`Your player · ${S.decodeName(S.encodeName(playerProfile.initials,playerProfile.badge))} · ${finalResult.score.toLocaleString()} points`;$('score-entry').open=false;
  ...
  async function loadLeaderboard(){
    ...
    const ticket=dialogGeneration,request=++leaderboardRequest;
    $('leaderboard-status').textContent='Loading scores…';$('refresh-scores').disabled=true;
    try{
      const entries=await S.leaderboard();if(ticket!==dialogGeneration||request!==leaderboardRequest)return;
    ...
    }catch{if(ticket===dialogGeneration&&request===leaderboardRequest)$('leaderboard-status').textContent=submission.done?'Your score was received, but the board could not refresh. Tap Refresh scores to try again.':$('leaderboard-list').children.length?'Showing the last loaded scores. Refresh failed; try again shortly.':'Could not connect to the score sheet after retrying. Your score is still here; try Refresh scores.';}
  ...
  function fitViewport(){
    ...
    if(!sideways&&window.matchMedia('(max-width:700px)').matches){
