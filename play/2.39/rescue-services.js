...
(function(root) {
  ...
  async function leaderboard() {
    ...
    for(let attempt=0;attempt<2;attempt++){
      try{
        data=await request(`${URL}?gameMode=${MODE}&limit=${LIMIT}&refresh=${Date.now()}-${attempt}`, {cache:'no-store'});
        if(!data.ok||!Array.isArray(data.entries))throw new Error('Could not load the shepherds’ board.');
        break;
      }catch(error){
        if(attempt===1)throw new Error('Could not refresh scores. Please try again shortly.');
