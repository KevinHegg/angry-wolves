(() => {
  ...
  function intro() {
    ...
    openDialog({view:'intro',eyebrow:'A SMALL ADVENTURE IN THREE CHAPTERS',title:'The gate was left open.',
      ...
      details:`<div class="daily-welcome"><b>Daily challenge · <span id="daily-date">${D.label(today())}</span></b><p>Same opening for everyone. Replay freely; your best posted score counts. New puzzle at midnight Eastern.</p><p id="yesterday-winner" role="status">Finding yesterday’s winner…</p>${saved?`<button id="start-daily-new" class="text-button" type="button">${sameDay?'Restart today’s challenge':`${saved.finalResult||saved.state.status==='lost'?'View':'Continue'} daily adventure · ${D.label(saved.challengeDate)}`}</button>`:''}</div><div class="instruction"><b>1.</b> Select 3+ matching animals touching side to side.</div><div class="instruction"><b>2.</b> Whistle them home. Herds of 3 or 4 bring the wolf one step closer; 5–6 hold it still; 7+ push it back.</div><div class="instruction"><b>3.</b> Fill the animal goals. Pip has 3 barks for the whole game: each sends the wolf back 3 steps and regroups animals. Open dots are available; filled dots are used.</div><p class="dialog-tip">Win without using Pip for +1,000 points. Any bark means no bonus. No clock—the wolf waits for your move. 🔊 Use the speaker button to turn sound on or off.</p>`,
      action:sameDay?`${saved.finalResult||saved.state.status==='lost'?'View':'Continue'} daily adventure · ${D.label(saved.challengeDate)}`:'Play today’s challenge',run:sameDay?resumeDaily:beginDaily,secondary:'Free play · a fresh random board',secondaryRun:beginFree });
    if(saved)$('start-daily-new').addEventListener('click',sameDay?beginDaily:resumeDaily);
    yesterdayWinner();
  }
  function render() {
    ...
    $('wolf-effect').textContent = R.pressure(state.moves+1)?`Whistle ${state.moves+1} · +${R.pressure(state.moves+1)} extra pressure`:`Whistle ${state.moves+1} · pressure from 18`;
  ...
  function showResults() {
    ...
    openDialog({view:'results',eyebrow:r.challengeDate?`DAILY CHALLENGE · ${D.label(r.challengeDate).toUpperCase()}`:'ADVENTURE COMPLETE · ALL THREE FIELDS',title:'Everyone is home.',copy:R.CHAPTERS[2].ending,
      ...
      action:'Play again · start at field 1',run:freshAdventure,secondary:'Choose daily challenge or free play',secondaryRun:intro});
