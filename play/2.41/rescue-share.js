(() => {
  ...
  async function makeCard(result, icon) {
    ...
    if (result.rank || result.personalBest) {
      ...
      ctx.fillText(result.rank ? `${result.rankBoard==='daily'?'DAILY HIGH SCORE':'TOP 20 HIGH SCORE'} · #${result.rank}` : 'NEW PERSONAL BEST',1117,84);
      if (result.rank) { ctx.font = '20px system-ui'; ctx.fillText(result.playerLabel,1117,115); }
      ctx.textAlign = 'left';
    }
    ctx.fillStyle = '#75765f'; ctx.font = '17px system-ui'; ctx.fillText(result.challengeDate?`DAILY CHALLENGE · ${result.challengeDate}`:'BRING THEM HOME',66,113);
    ctx.fillStyle = '#263a33'; ctx.font = '58px Georgia'; ctx.fillText('Home safe. Wolf hungry.',66,195);
  ...
  async function share(result, card) {
    ...
    const file = card ? new File([card.blob], 'hungry-wolf-score.png', {type:'image/png'}) : null;
    const data = file && navigator.canShare?.({files:[file]})
      ? {title:'Hungry Wolf', files:[file], url:S.GAME_URL}
      : {title:'Hungry Wolf', url:S.GAME_URL};
