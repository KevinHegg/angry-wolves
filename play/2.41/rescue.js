(() => {
  ...
  function openDialog({eyebrow,title,copy,details='',action,run,secondary='',secondaryRun,view=''}) {
    ...
    $('load-version').hidden=view!=='intro';
  ...
  function saveDaily(){
    ...
    try{localStorage.setItem('hw-daily-adventure',JSON.stringify({ruleset:D.RULESET,challengeDate,dailyStartedAt,state,randoms:randoms.snapshot(),total,bankedScore,bankedMoves,bankedBiggest,pipUsed,startedAt,runNonce,finalResult,submission:{...submission,pending:false}}));}catch{}
  ...
  function beginDaily(){challengeDate=today();dailyStartedAt=Date.now()+serverOffset;freshAdventure();tone('gate');}
  function beginFree(){challengeDate='';freshAdventure();tone('gate');}
  function resumeDaily(){
    const v=savedDaily();if(!v){beginDaily();return;}
  ...
  async function yesterdayWinner(){
    ...
    try{const data=await S.board('daily');serverOffset=Number.isFinite(data.serverTime)?data.serverTime-Date.now():0;
      ...
      const e=data.yesterday;$('yesterday-winner').textContent=e?`Yesterday’s winner · ${S.decodeName(e.playerName)} · ${e.score.toLocaleString()} points`:'A fresh tradition: no daily winner yesterday.';
      $('daily-date').textContent=D.label(data.today||today());
    }catch{if(ticket===dialogGeneration)$('yesterday-winner').textContent='Yesterday’s winner is unavailable right now.';}
  }
  function intro() {
    ready = false;
    const saved=savedDaily(),sameDay=saved?.challengeDate===today();
    const won=saved?.state.status==='won'&&saved.state.chapter===2;
    // Migrate completed v2.40 saves, including a reload during the ending animation.
    ...
    const savedLabel=saved?`${saved.finalResult||won||saved.state.status==='lost'?'View':'Continue'} daily adventure · ${D.label(saved.challengeDate)}`:'';
    const replayLabel=completed?'Replay today’s challenge':'Restart today’s challenge';
    openDialog({view:'intro',eyebrow:'A SMALL ADVENTURE IN THREE CHAPTERS',title:'The gate was left open.',
      copy:'The animals wandered out. The wolf noticed. Bring the herds home with a whistle, and a little help from Pip the sheepdog.',
      details:`<div class="daily-welcome"><b>Daily challenge · <span id="daily-date">${D.label(today())}</span></b><p>${completed?`✓ Completed today · best ${completed.score.toLocaleString()} points. Play free or choose a daily replay. Next challenge at midnight Eastern.`:'Same opening for everyone. Replay freely; your best posted score counts. New puzzle at midnight Eastern.'}</p><p id="yesterday-winner" role="status">Finding yesterday’s winner…</p>${saved||completed?`<button id="start-daily-new" class="text-button" type="button">${sameDay||!saved?replayLabel:savedLabel}</button>`:''}</div><div class="instruction"><b>1.</b> Select 3+ matching animals touching side to side.</div><div class="instruction"><b>2.</b> Whistle them home. For the first 17 whistles in each field: 3–4 animals bring the wolf one step closer; 5–6 hold it still; 7+ push it back. From whistle 18, add one step closer.</div><div class="instruction"><b>3.</b> Fill the animal goals. Pip has 3 barks for the whole game: each sends the wolf back 3 steps and regroups animals. Open dots are available; filled dots are used.</div><p class="dialog-tip">Win without using Pip for +1,000 points. Any bark means no bonus. No clock—the wolf waits for your move. 🔊 Use the speaker button to turn sound on or off.</p>`,
      action:completed?'Free play · a fresh random board':sameDay?savedLabel:'Play today’s challenge',run:completed?beginFree:sameDay?resumeDaily:beginDaily,
      secondary:completed?(sameDay?savedLabel:''):'Free play · a fresh random board',secondaryRun:completed?resumeDaily:beginFree });
    if(saved||completed)$('start-daily-new').addEventListener('click',sameDay||!saved?beginDaily:resumeDaily);
    yesterdayWinner();
  }
  function render() {
    const c = R.CHAPTERS[state.chapter];
    $('retry').textContent=challengeDate?'Give up':'Restart game';
    document.body.className = `chapter-${state.chapter}`;
    $('chapter-label').textContent = challengeDate?`DAILY ${D.label(challengeDate).toUpperCase()} · FIELD ${state.chapter+1}`:`CHAPTER ${chapterWords[state.chapter]} · ${c.time.toUpperCase()}`;
  ...
  function select(i) {
    if(sideways||!ready||busy||state.status!=='playing'||$('story-dialog').open)return;
  ...
  function commit() {
    if(sideways||!ready||busy||!selected.length||state.status!=='playing'||$('story-dialog').open)return;
    busy=true;
    ...
    afterMove(()=>{
      ...
      if(busy&&state.status==='playing')afterMove(()=>{if(ticket!==generation)return;busy=false;render();},2500);
      ...
      if(state.status!=='playing'){
        if(gameOver){
          ...
          afterMove(()=>{
            ...
            afterMove(()=>{if(ticket!==generation)return;busy=false;finishField(true);},2000);
          },wolfTravel);
        }else afterMove(()=>{if(ticket!==generation)return;busy=false;finishField();},reducedMotion.matches?350:windRotations.length?2700:900);
      }
  ...
  function freshAdventure() {
    ...
    if(challengeDate){challengeDate=today();dailyStartedAt=Date.now()+serverOffset;}
  ...
  function retryOrGiveUp(){
    ...
    if(shareCard)URL.revokeObjectURL(shareCard.url);shareCard=null;shareReady=false;
    renderSelection();intro();say('Daily challenge ended. Choose today’s challenge or free play when you are ready.');
  }
  function leaveFinishedAdventure(){ready=false;closeDialog();say('Adventure over. Rest here, or choose Restart game for a new journey.');}
  function finishField(endingPresented=false) {
    if(state.status==='playing')return;
    busy=false;
    if(state.status==='lost'){
      ready=false;if(!endingPresented)tone('howl-deep');
      openDialog({view:'lost',eyebrow:'ADVENTURE OVER · THE ANIMALS ARE SHELTERING',title:'The wolf caught up.',
        copy:'Pip led the animals to safety. This adventure has ended. Start a new journey from field one, or leave the game here.',
        action:challengeDate?'Retry daily challenge':'Play again · start at field 1',run:freshAdventure,secondary:'Not now',secondaryRun:leaveFinishedAdventure});return;
    }
    ...
    openDialog({eyebrow:`FIELD ${state.chapter+1} OF 3 COMPLETE`,title:['The sheep are safe.','Out of the orchard.'][state.chapter],copy:`${R.CHAPTERS[state.chapter].ending} ${next.story}`,
      details:`<div class="stat-line"><span><strong>${total+state.saved.reduce((a,b)=>a+b,0)}</strong>animals home</span><span><strong>${3-state.bark} of 3</strong>Pip barks used · ${state.bark?`${state.bark} left`:"no barks left"}</span></div><p class="dialog-tip">Next: ${next.name}. Your animals carry forward. The wolf stays ${state.distance} ${state.distance===1?'step':'steps'} away. Pip has ${state.bark} bark${state.bark===1?'':'s'} left for the game. Dust devils wait three rescues, scatter surrounding animals, then settle into an animal. Pip leaves the gust and its countdown unchanged.</p>`,
  ...
  function showLeaderboard(view) {
    if(busy)return;
    if(typeof view==='string'&&['daily','weekly','alltime','winners'].includes(view))boardView=view;
    ...
    const form=finalResult&&!submission.done?`<form id="score-form"><p id="player-lock-status" class="dialog-tip">${locked?'Saved player locked. Your previous scores keep their original name.':'Choose a player for this and future scores. Previous scores keep their original name.'}</p><fieldset id="player-fields" class="player-fields" ${locked||submission.pending?'disabled':''}><span class="initials-label">Your three letters</span><input id="score-initials" type="hidden"><div class="letter-wheels" role="group" aria-label="Three-letter player name">${[0,1,2].map(i=>`<div class="letter-wheel"><button type="button" data-letter="${i}" data-step="1" aria-label="Next letter ${i+1}">▲</button><output id="player-letter-${i}" aria-live="polite">${(initials||'AAA')[i]}</output><button type="button" data-letter="${i}" data-step="-1" aria-label="Previous letter ${i+1}">▼</button></div>`).join('')}</div><fieldset class="badge-picker"><legend>Choose your shepherd badge</legend>${badgeChoices.map(i=>`<label><input type="radio" name="shepherd-badge" value="${i}" ${i===badge?'checked':''}><span title="${S.BADGE_NAMES[i]}" aria-label="${S.BADGE_NAMES[i]}">${S.BADGES[i]}</span></label>`).join('')}</fieldset></fieldset></form>`:'';
    openDialog({view:'leaderboard',eyebrow:'THE SHEPHERDS’ BOARD',title:'The top shepherds',copy:'Daily adventures, weekly totals and the all-time greats.',
      details:`<div class="leaderboard-tabs" role="group" aria-label="Leaderboard period">${[['daily','Daily'],['weekly','Weekly'],['alltime','All time'],['winners','Daily winners']].map(([id,label])=>`<button type="button" data-board="${id}" aria-pressed="${boardView===id}">${label}</button>`).join('')}</div><p id="board-description" class="board-description"></p>${form?`<details id="score-entry" class="score-entry" ${locked?'':'open'}><summary id="player-summary">Your player · ${locked?S.decodeName(S.encodeName(initials,badge))+' · ':''}${finalResult.score.toLocaleString()} points</summary>${form}</details>`:''}<p id="leaderboard-status" role="status">Loading scores…</p><ol id="leaderboard-list" class="leaderboard-list" tabindex="0" aria-label="Top scores. Scroll for more."></ol><button id="refresh-scores" class="text-button" type="button">Refresh scores</button>`,
      action:finalResult?'Back to my score':state.status==='playing'?'Back to the field':'Back to finished game',run:finalResult?showResults:closeDialog});
    ...
    if($('score-initials')){
      ...
      $('toggle-player-lock').onclick=()=>{
        ...
        if($('player-fields').disabled){
          ...
          profileUnlocked=false;rememberProfile();$('submit-status').textContent='Player saved for this and future games.';$('player-fields').disabled=true;$('toggle-player-lock').textContent='Change player';
          $('player-lock-status').textContent='Saved player locked.';
          $('player-summary').textContent=`Your player · ${S.decodeName(S.encodeName(playerProfile.initials,playerProfile.badge))} · ${finalResult.score.toLocaleString()} points`;$('score-entry').open=false;$('dialog-details').scrollTop=0;
  ...
  async function loadLeaderboard(){
    ...
    $('board-description').textContent={daily:`${D.label(date)} · best score per player. Post before midnight Eastern.`,weekly:`Week of ${D.label(D.weekStart(date))} · sum of each player’s best daily scores, Monday–Sunday.`,alltime:'Top 20 adventures, including scores earned under earlier house rules.',winners:'All-time daily winners · one champion per completed day, ranked by winning score.'}[view];
    ...
    try{
      ...
      entries.forEach((entry,i)=>{
        ...
        herd.textContent=view==='weekly'?`${entry.daysPlayed} of 7 daily scores · biggest herd ${entry.biggestHerdCount} ${animal}`:`${view==='winners'?D.label(entry.challengeDate,true)+' · ':''}Biggest herd ${Math.max(0,Math.min(36,Number(entry.biggestHerdCount)||0))} ${animal}`;
      ...
      if(finalResult && !submission.done && $('submit-score')) {
        ...
        $('submit-status').textContent=eligible ? submission.message || (finalResult.challengeDate?'Post this daily score. Your best today counts toward the week.':'Post your adventure to the all-time board.') : `This time you need more than ${entries[S.LIMIT-1].score.toLocaleString()} points to enter the top 20.`;
      }
    ...
    finally{if(ticket===dialogGeneration&&request===leaderboardRequest)$('refresh-scores').disabled=false;}
  }
  async function postScore(event){
    ...
    try{
      const entries=result.challengeDate?(await S.board('daily',result.challengeDate)).entries:await S.leaderboard();
      ...
      if(dialogView==='leaderboard'){$('submit-score').hidden=true;$('toggle-player-lock').hidden=true;$('score-form')?.remove();$('submit-status').textContent=submission.message;loadLeaderboard();}
  ...
  $('bark').addEventListener('click',()=>{
    if(sideways||!ready||busy||$('story-dialog').open||!state.bark||state.status!=='playing')return;
  ...
  window.addEventListener('pageshow',event=>{if(event.persisted&&dialogView==='intro')intro();scheduleFit();},{passive:true});
