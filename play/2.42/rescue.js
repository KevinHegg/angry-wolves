(() => {
  ...
  function showLeaderboard(view) {
    ...
    openDialog({view:'leaderboard',eyebrow:'THE SHEPHERDS’ BOARD',title:'The top shepherds',copy:'Daily, weekly and all-time scores.',
      details:`<div class="leaderboard-tabs" role="group" aria-label="Leaderboard period">${[['daily','Daily'],['weekly','Weekly'],['alltime','All time'],['daily-alltime','Daily records'],['winners','Daily winners']].map(([id,label])=>`<button type="button" data-board="${id}" aria-pressed="${boardView===id}">${label}</button>`).join('')}</div><p id="board-description" class="board-description"></p>${form?`<details id="score-entry" class="score-entry" ${locked?'':'open'}><summary id="player-summary">Your player · ${locked?S.decodeName(S.encodeName(initials,badge))+' · ':''}${finalResult.score.toLocaleString()} points</summary>${form}</details>`:''}<p id="leaderboard-status" role="status">Loading scores…</p><ol id="leaderboard-list" class="leaderboard-list" tabindex="0" aria-label="Top scores. Scroll for more."></ol><button id="refresh-scores" class="text-button" type="button">Refresh scores</button>`,
