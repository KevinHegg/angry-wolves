(() => {
  ...
  function intro() {
    ...
    const availableSave=saved&&dailyAttempt(saved.challengeDate)?.status!=='gave-up';
    const savedLabel=availableSave?`${saved.finalResult||saved.state.status==='lost'||(saved.state.status==='won'&&saved.state.chapter===2)?'View':'Continue'} daily adventure · ${D.label(saved.challengeDate)}`:'';
    const continuing=sameDay&&availableSave&&attempt?.status==='active';
    const dailyMessage=completed?`✓ Completed today · best ${completed.score.toLocaleString()} points. Your attempt is complete.`:continuing?'Your one attempt is in progress. Continue exactly where you left off.':attempt?'Today’s attempt is over. Come back tomorrow for a new challenge.':'One attempt per day. Starting uses your attempt; a loss or Give up ends it. You can close the game and resume.';
  ...
  function retryOrGiveUp(){
    ...
    if(state.status!=='playing'){intro();return;}
    ...
    if(shareCard)URL.revokeObjectURL(shareCard.url);shareCard=null;shareReady=false;
    render();intro();say('Daily attempt ended. Free play is always available; the next challenge arrives at midnight Eastern.');
  }
  ...
  function finishField(endingPresented=false) {
    ...
    openDialog({eyebrow:`${challengeDate?'DAILY CHALLENGE · ':''}FIELD ${state.chapter+1} OF 3 COMPLETE`,title:['The sheep are safe.','Out of the orchard.'][state.chapter],copy:`${R.CHAPTERS[state.chapter].ending} ${next.story}`,
  ...
  async function loadLeaderboard(){
    ...
    try{
      ...
      if(finalResult && !submission.done && $('submit-score')) {
        ...
        $('submit-status').textContent=eligible ? submission.message || (finalResult.challengeDate?'Post your daily result to count toward the week.':'Post your adventure to the all-time board.') : `This time you need more than ${entries[S.LIMIT-1].score.toLocaleString()} points to enter the top 20.`;
      }
  ...
  window.addEventListener('storage',event=>{if(event.key?.startsWith('hw-daily-attempt:')){if(challengeDate)dailyCurrent();else if(dialogView==='intro')intro();}});
  window.addEventListener('pageshow',event=>{if(event.persisted){if(challengeDate)dailyCurrent();if(dialogView==='intro')intro();}scheduleFit();},{passive:true});
