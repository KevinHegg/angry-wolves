(() => {
  ...
  let profileUnlocked=false,leaderboardRequest=0,boardDate='',boardScope='standings',boardPoll=null,boardPending='',boardKey='';
  ...
  function render() {
    ...
    wolfTrail.classList.toggle('pressure-one',R.pressure(state.moves)===1);
    wolfTrail.classList.toggle('pressure-two',R.pressure(state.moves)===2);
  ...
  function showLeaderboard(view) {
    ...
    $('board-date').addEventListener('change',event=>{boardScope=event.target.value==='records'?'records':'standings';boardDate=boardScope==='records'?'':event.target.value;loadLeaderboard();});
  ...
  function renderStandings(data){
    ...
    data.entries.forEach(entry=>list.append(standingRow(entry)));list.style.setProperty('--standings-height',`${48*Math.min(5,Math.max(1,data.entries.length))}px`);list.scrollTop=scroll;
    $('board-date-control').hidden=boardView!=='daily';
    const picker=$('board-date'),choice=boardScope==='records'?'records':boardDate===data.today?'':boardDate;picker.replaceChildren();
    for(const [value,label] of [['',`Today · ${D.label(data.today)}`],...data.dates.filter(day=>day!==data.today).map(day=>[day,D.label(day,true)]),['records','Daily records · all dates']]){
