(() => {
  ...
  function render() {
    ...
    $('retry').textContent=challengeDate?(state.status==='lost'||finalResult?'Free play':'Give up'):consecutiveRestarts>=MAX_CONSECUTIVE_RESTARTS?'Play a herd to restart again':'Restart game';
    $('retry').disabled=!challengeDate&&consecutiveRestarts>=MAX_CONSECUTIVE_RESTARTS;
    $('retry').title=$('retry').disabled?'Three restarts used. Whistle a herd home to restart again.':'';
  ...
  function commit() {
    ...
    afterMove(()=>{
      ...
      if(result.ok&&!challengeDate)consecutiveRestarts=0;
