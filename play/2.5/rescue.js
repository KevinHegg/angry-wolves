(() => {
  ...
  function startField(chapter, carriedBoard = null) {
    generation++;busy=false;selected=[];ready=true;
    state=R.create(chapter,Math.random,carriedBoard);fieldEntryBoard=state.board.slice();render();window.scrollTo(0,0);
  ...
  function finishField() {
    if(state.status==='lost'){
      openDialog({eyebrow:'PIP GUIDED THE HERDS TO COVER',title:'A little too close.',copy:'The wolves reached the field. The animals are sheltering with Pip. Try again: three animals bring the pack two steps closer, while five or more hold them off.',
        ...
        action:'Try this field again',run:()=>{closeDialog();startField(state.chapter,fieldEntryBoard);} });return;
    }
    tone('win');
    if(state.chapter===2){
    ...
    openDialog({eyebrow:`FIELD ${state.chapter+1} OF 3 COMPLETE`,title:['The sheep are safe.','Out of the orchard.'][state.chapter],copy:`${R.CHAPTERS[state.chapter].ending} ${next.story}`,
      details:`<div class="stat-line"><span><strong>${total+state.saved.reduce((a,b)=>a+b,0)}</strong>animals home</span><span><strong>${rested}</strong>fields Pip rested</span></div><p class="dialog-tip">Next: ${next.name}. Your animals carry forward. Pip gets a fresh bark.</p>`,
      action:`Continue to field ${state.chapter+2}`,run:()=>{
        ...
        const chapter=state.chapter+1,board=state.board.slice();closeDialog();tone('gate');startField(chapter,board);
      }});
  }
  ...
  $('bark').addEventListener('click',()=>{
    ...
    pipUsed[state.chapter]=true;selected=[];tone('bark');if(state.distance>previousDistance)tone('whimper',{delay:.4});render();say('Good dog! Wolves back 3 steps. The animals regrouped.');
  });
