(() => {
  ...
  function render() {
    ...
    $('wolf-effect').textContent = extra
      ? `Next whistle ${nextWhistle} · 5–6: ${extra} closer`
      : `Next whistle ${nextWhistle} · 3–4: 1 closer`;
    $('wolf-effect').classList.toggle('extra-pressure',extra>0);
    $('wolf-effect').setAttribute('aria-label',`Next whistle ${nextWhistle}. A herd of 3 or 4 moves the wolf ${1+extra} ${1+extra===1?'step':'steps'} closer. A herd of 5 or 6 moves it ${extra?`${extra} ${extra===1?'step':'steps'} closer`:'neither way'}. A herd of 7 or more moves it ${extra===0?'1 step back':extra===1?'neither way':'1 step closer'}.`);
  ...
  function select(i) {
    ...
    const blast=R.burstPreview(state),ruleStep=R.wolfStep(g.length)-R.pressure(state.moves+1)-blast.cats.length;
    ...
    const effect=gateShuts?(state.distance===0?'The last gate shuts and the wolf backs away.':'The last gate closes before the wolf can move.'):step>0?'The wolf steps back!':step===0?'The wolf stays put.':`The wolf moves ${-step} ${step===-1?'step':'steps'} closer${state.distance<=-step?' — finish the goal!':'.'}`;
