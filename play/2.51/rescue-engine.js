...
(function (root) {
  ...
  const REST_BONUSES = Object.freeze([0, 0, 350, 1000]);
  function restBonus(rested) { return REST_BONUSES[Math.max(0, Math.min(3, Math.floor(rested)))]; }
  function wolfStep(count) { return count <= 4 ? -1 : count < 7 ? 0 : 1; }
  function herdPoints(count) { return count * 10 + Math.max(0, count - 3) ** 2 * 2; }
  const PRESSURE_THRESHOLDS = Object.freeze([15, 30]);
  function pressure(moves) { return moves >= PRESSURE_THRESHOLDS[1] ? 2 : moves >= PRESSURE_THRESHOLDS[0] ? 1 : 0; }
  function wolfMove(count, whistle) {
    const level=pressure(whistle);
    if(level===2)return -2;
    if(level===1)return count<=5?-2:0;
    return wolfStep(count);
  }
  const CHAPTERS = [
    { name: 'The open pasture', time: 'Late afternoon', goal: [14, 0, 0, 0], types: 3, distance: 5,
      story: 'The gate is open. The sheep have wandered out. Get 14 sheep home before the wolf reaches the field.',
    ...
    { name: 'Trouble in the orchard', time: 'Sunset', goal: [0, 18, 18, 0], types: 4, distance: 6,
      story: 'The pigs and hens heard the howling. They are hiding in the orchard. Bring 18 of each back to the lane. Cows join as spaces refill—build their herds for the last gate.',
    ...
    { name: 'The last gate', time: 'Nightfall', goal: [14, 14, 14, 14], types: 4, distance: 5,
      story: 'The barn lantern is lit. The wolf is close. Bring 14 of every animal home, then shut the last gate.',
      ending: 'The last gate swings shut. Every herd is home. The wolf will have to go hungry tonight.' }
  ];
  ...
  function ensureMove(board, types, rng) {
    ...
    // Regroup existing animals: Pip and dead-board recovery never change species counts.
    const counts=Array.from({length:types},(_,t)=>board.filter(v=>v===t).length);
    const choices=counts.flatMap((n,t)=>n>=3?[t]:[]);
    const first=[30,33,24,27,18,21,12,15,6,9,0,3].find(i=>![board[i],board[i+1],board[i+2]].some(isSpecial));
    if(first===undefined||!choices.length)return false;
    const type=choices[Math.floor(rng()*choices.length)],targets=[first,first+1,first+2];
  ...
  function openingBoard(rng) {
    ...
    const mixes=[];
    for(let a=11;a<=13;a++)for(let b=11;b<=13;b++){
      const c=36-a-b;
      if(c>=11&&c<=13&&![a,b,c].every(n=>n===12))mixes.push([a,b,c]);
    }
    ...
    if(!groups(board).length){
      ...
      const type=animal(3,rng),row=Math.floor(rng()*6),col=Math.floor(rng()*4);
  ...
  function create(chapter = 0, rng = Math.random, carriedBoard = null, carriedCats = {}, settings = WIND_SETTINGS, carriedWait = null, carriedBarks = 3) {
    const c = CHAPTERS[chapter], windRng=rng.wind||rng; rng=rng.animals||rng;
    ...
    return { chapter, board, windWait:carriedWait??windWait(windRng), catSettings:settings, cats:Object.fromEntries(board.flatMap((t,i)=>isSpecial(t)?[[i,Math.min(3,Math.max(1,carriedCats[i]||3))]]:[])),catCooldown:0, saved: [0, 0, 0, 0], distance: c.distance, bark: carriedBarks, moves: 0, score: 0, biggest: {count: 0, type: 0}, status: 'playing' };
  }
  function rescue(state, start, rng = Math.random) {
    if (state.status !== 'playing') return { ok: false };
    const settings=state.catSettings||CAT_SETTINGS, windRng=rng.wind||rng, regroupRng=rng.regroup||rng; rng=rng.animals||rng;
    ...
    const goalsMet=CHAPTERS[state.chapter].goal.every((n,i)=>state.saved[i]>=n);
    let step = wolfMove(count,state.moves) - preview.cats.length;
    // The last gate closes before a winning whistle could put the wolf in the pen.
    if(goalsMet&&state.chapter===CHAPTERS.length-1&&state.distance+step<=0)step=Math.max(1,state.distance)-state.distance;
    ...
    for(const i of Object.keys(state.cats).map(Number))if(state.board[i]===DUST){
      if(state.cats[i]===0){
        const moves=scatterWind(state.board,i,windRng);rotations.push({index:i,moves,completed:3});
        const gift=windGift(state,i,windRng);state.board[i]=gift.type;delete state.cats[i];gifts.push(gift);
      }
    }
    ...
    // A last-turn rescue succeeds before the wolf can enter the field.
    if (goalsMet) state.status = 'won';
    else if (state.distance <= 0) state.status = 'lost';
    let catAppeared=-1;
    if(settings.kind==='wind'){
      if(gifts.length)state.windWait=windWait(windRng);
        ...
        if(state.windWait<=0){
          ...
          if(top.length){catAppeared=top[Math.floor(windRng()*top.length)];state.board[catAppeared]=DUST;state.cats[catAppeared]=3;}
    ...
    const regrouped = ensureMove(state.board, CHAPTERS[state.chapter].types, regroupRng);
  ...
  function bark(state, rng = Math.random) {
    ...
    const animalRng=rng.animals||rng,windRng=rng.wind||rng; rng=rng.regroup||rng;
    state.bark--;
    ...
    if(catIndices.length){
      collapse(state,new Set(catIndices),animalRng);state.windWait=windWait(windRng);state.catCooldown=CAT_SETTINGS.cooldown;
  ...
  const api = { SIZE, CAT, GOOD_CAT, DUST, isCat, isSpecial, WIND_SETTINGS, windRing, scatterWind, windGift, bestGift, catNeighbors, CAT_SETTINGS, CHAPTERS, REST_BONUSES, PRESSURE_THRESHOLDS, restBonus, wolfStep, wolfMove, herdPoints, pressure, neighbors, group, groups, create, rescue, bark, burstPreview };
