(() => {
  ...
  async function makeCard(result, icon) {
    ...
    await token.decode();
    const daily = !!result.challengeDate;
    ...
    ctx.fillStyle = '#263a33'; ctx.font = '58px Georgia'; ctx.fillText('Home safe. Wolf hungry.',66,daily?219:195);
    ...
    ctx.font = '21px system-ui'; ctx.fillStyle = '#63705d'; ctx.fillText(`${result.rested===3?'No Pip used':`${result.rested} Pip barks saved`} · +${result.bonus.toLocaleString('en-US')} bonus`,70,461);
    ctx.fillStyle = '#d7caa9'; ctx.fillRect(66,521,1051,1);
    ctx.font = 'bold 20px system-ui'; ctx.fillStyle = '#315d45'; ctx.fillText(daily?'ONE DAILY CHALLENGE. ONE CHANCE.':'CAN YOU BEAT MY HERD?',66,564);
  ...
  async function share(result, card) {
    ...
    const file = card ? new File([card.blob], result.challengeDate?`hungry-wolf-daily-${result.challengeDate}.png`:'hungry-wolf-score.png', {type:'image/png'}) : null;
    const title=result.challengeDate?'Hungry Wolf · Daily Challenge':'Hungry Wolf';
    const data = file && navigator.canShare?.({files:[file]})
      ? {title, files:[file], url:S.GAME_URL}
      : {title, url:S.GAME_URL};
