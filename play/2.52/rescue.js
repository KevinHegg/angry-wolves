(() => {
  ...
  window.RescueIcon = icon; // lets the unpublished poster page draw the same animal art
  function icon(type) {
  ...
  let challengeDate="",dailyStartedAt=0,randoms=Math.random,boardView="daily",serverOffset=0;
  ...
  let profileUnlocked=false,leaderboardRequest=0,boardDate='',boardScope='standings',boardPoll=null,boardPending='',boardKey='',boardDates=[];
  ...
  function openDialog({eyebrow,title,copy,details='',action,run,secondary='',secondaryRun,view='',back=null}) {
    stopBoardRefresh();dialogGeneration++; dialogView = view;dialogBack=back;
  ...
  function closeDialog() {
    hiddenDialog=false;dialogScroll=[];stopBoardRefresh();
  ...
  function saveDaily(){
    ...
    if(state.status==='won'&&state.chapter===2)rememberDailyWin(challengeDate,finalResult?.score??bankedScore+state.score+R.restBonus(state.bark));
  }
  function savedDaily(){
  ...
  function migrateDaily(){
    ...
    const completed=rememberDailyWin(won?saved.challengeDate:null,won?(saved.finalResult?.score??saved.bankedScore+saved.state.score+R.restBonus(saved.state.bark)):null);
  ...
  async function yesterdayWinner(){
    ...
    try{const data=await S.consolidatedBoard('daily');if(data.authoritative)serverOffset=data.serverTime-Date.now();
  ...
  function render() {
    ...
    $('retry').disabled=!challengeDate&&consecutiveRestarts>=MAX_CONSECUTIVE_RESTARTS;
    $('retry').title='';
    ...
    $('pip-rest-hint').textContent = state.bark===3?'No-Pip bonus: +1,000':state.bark===2?'Pip bonus: +350':'Pip bonus: 0';
    ...
    $('wolf-effect').textContent = extra===2
      ? `Next whistle ${nextWhistle} · any herd: 2 closer`
      : extra===1
        ? `Next whistle ${nextWhistle} · 3–5: 2 closer`
        : `Next whistle ${nextWhistle} · 3–4: 1 closer`;
    $('wolf-effect').classList.toggle('extra-pressure',extra>0);
    $('wolf-effect').setAttribute('aria-label',extra===2?`Next whistle ${nextWhistle}. Every herd moves the wolf 2 steps closer.`:extra===1?`Next whistle ${nextWhistle}. Herds of 3 to 5 move the wolf 2 steps closer. Herds of 6 or more hold it still.`:`Next whistle ${nextWhistle}. Herds of 3 or 4 move the wolf 1 step closer, 5 or 6 hold it still, and 7 or more push it back 1 step.`);
    const wolfTrail=document.querySelector('.wolf-trail');
    wolfTrail.classList.toggle('danger',state.distance<=2);
    wolfTrail.classList.toggle('pressure-one',extra===1);
    wolfTrail.classList.toggle('pressure-two',extra===2);
  ...
  function pressureWarning(){
    ...
    return extra===2?`From whistle ${n}: every herd moves the wolf 2 steps closer.`:`From whistle ${n}: 3–5 animals move the wolf 2 steps closer; 6+ hold it still.`;
  }
  function select(i) {
    ...
    const blast=R.burstPreview(state),ruleStep=R.wolfMove(g.length,state.moves+1)-blast.cats.length;
    const completesLastGoal=state.chapter===2&&R.CHAPTERS[2].goal.every((goal,type)=>state.saved[type]+(state.board[i]===type?g.length:0)+blast.animals.filter(cell=>!g.includes(cell)&&state.board[cell]===type).length>=goal);
    const gateShuts=completesLastGoal&&state.distance+ruleStep<=0;
    const step=gateShuts?Math.max(1,state.distance)-state.distance:Math.max(0,Math.min(10,state.distance+ruleStep))-state.distance;
    const effect=gateShuts?(state.distance===0?'The last gate shuts and the wolf backs away.':'The last gate closes before the wolf can move.'):state.distance+ruleStep<=0?'The wolf reaches the pen!':step>0?'The wolf steps back!':step===0?'The wolf stays put.':`The wolf moves ${-step} ${step===-1?'step':'steps'} closer.`;
  ...
  function commit() {
    ...
    afterMove(()=>{
      ...
      say(`${result.count} ${names[result.type]} home! ${state.status==='lost'?'The wolf reached the pen.':moved>0?'The wolf backs away.':moved===0?'The wolf stays put.':`The wolf moves ${-moved} ${moved===-1?'step':'steps'} closer.`}${result.catAppeared>=0?' A dust devil rolls in!':''}${windRotations.length?' Whoosh! Nearby animals scatter.':''}${!gameOver&&result.gifts.length?` The wind settles into ${names[result.gifts[0].type]}, making a herd of ${result.gifts[0].size}.`:''}${result.regrouped?' A new herd gathered.':''}`);
  ...
  function startField(chapter, carriedBoard = null,cats={},distance=R.CHAPTERS[chapter].distance,wait=null,barks=3) {
    ...
    if(chapter>0)consecutiveRestarts=0;
  ...
  function freshAdventure(newDaily=false,fromRestart=false) {
    if(sideways)return;
    if(challengeDate&&!newDaily){beginDaily();return;}
    if(!fromRestart)consecutiveRestarts=0;
  ...
  function retryOrGiveUp(){
    ...
    if(!challengeDate){if(consecutiveRestarts>=MAX_CONSECUTIVE_RESTARTS)return;consecutiveRestarts++;freshAdventure(false,true);return;}
  ...
  function finishField(endingPresented=false) {
    ...
    if(state.chapter===2){
      const rested=state.bark,bonus=R.restBonus(rested);
  ...
  function showResults() {
    ...
    openDialog({view:'results',eyebrow:r.challengeDate?`DAILY CHALLENGE · ${D.label(r.challengeDate).toUpperCase()}`:'ADVENTURE COMPLETE · ALL THREE FIELDS',title:'Everyone is home.',copy:R.CHAPTERS[2].ending,
      details:`<div class="stat-line"><span><strong>${r.saved}</strong>animals home</span><span><strong>${r.score.toLocaleString()}</strong>total points</span></div><p id="result-status" class="dialog-tip" role="status"></p><p class="score-breakdown">${r.herdingScore.toLocaleString()} herding + <b>${r.bonus.toLocaleString()} Pip bonus</b><br>${r.rested===3?'No Pip used':`${r.rested} Pip bark${r.rested===1?'':'s'} saved`} · personal best ${best.toLocaleString()}</p><div class="biggest-herd">${icon(r.biggest.type)}<span>Biggest herd: <b>${r.biggest.count} ${names[r.biggest.type]}</b></span></div><p id="share-status" class="dialog-tip" role="status"></p><button id="copy-caption" class="text-button" type="button">Copy score &amp; link</button><span id="image-tools" hidden><button id="copy-image" class="text-button" type="button">Copy image</button><button id="save-image" class="text-button" type="button">Save image</button></span><details><summary>Preview share image</summary><img id="share-preview" class="share-preview" alt="Your Hungry Wolf score card" hidden></details>`,
    ...
    $('share-score').addEventListener('click',async()=>{
      ...
      try{
        ...
        if(ticket!==dialogGeneration)return;
        $('share-status').textContent=outcome==='shared'?'Your share is ready.':outcome==='cancelled'?'Sharing cancelled. Your score is still here.':'Sharing is unavailable in this browser. Use Copy score & link or Save image.';
      }catch{if(ticket===dialogGeneration)$('share-status').textContent='Sharing could not open. Try again or use Copy score & link.';}
    ...
    $('copy-caption').addEventListener('click',async()=>{
      try{await navigator.clipboard.writeText(S.caption(r));if(ticket===dialogGeneration)$('share-status').textContent='Score and link copied.';}
      catch{if(ticket===dialogGeneration)$('share-status').textContent=S.caption(r);}
    });
    $('copy-image').addEventListener('click',async()=>{
      try{const copied=await window.RescueShare.copyImage(shareCard);if(ticket===dialogGeneration)$('share-status').textContent=copied?'Score image copied.':'Copying images is unavailable here. Use Save image.';}
      catch{if(ticket===dialogGeneration)$('share-status').textContent='The image could not be copied. Use Save image.';}
    });
    $('save-image').addEventListener('click',()=>{
      try{window.RescueShare.saveImage(r,shareCard);$('share-status').textContent='Your score image is downloading.';}
      catch{$('share-status').textContent='The image could not be saved.';}
    });
    attachShare();
  }
  ...
  function attachShare(){
    ...
    $('image-tools').hidden=window.RescueShare.canShareImage(finalResult,shareCard);$('copy-image').hidden=!window.RescueShare.canCopyImage();
  ...
  function showLeaderboard(view) {
    ...
    boardView=view==='alltime'?'alltime':'daily';boardDate='';boardScope='standings';boardKey='';
    ...
    openDialog({view:'leaderboard',eyebrow:'THE SHEPHERDS’ BOARD',title:'Leaderboards',copy:'',
      details:`<div class="leaderboard-tabs" role="tablist" aria-label="Leaderboards">${[['daily','Daily Challenge'],['alltime','All-Time']].map(([id,label])=>`<button id="board-tab-${id}" type="button" role="tab" data-board="${id}" aria-selected="${boardView===id}" aria-controls="board-panel" tabindex="${boardView===id?0:-1}">${label}</button>`).join('')}</div><div id="board-panel" role="tabpanel" aria-labelledby="board-tab-${boardView}"><div id="board-date-control" aria-label="Challenge date"><button id="board-date-prev" class="board-date-arrow" type="button" aria-label="Previous challenge">‹</button><button id="board-date-toggle" type="button" aria-label="Choose challenge date" aria-expanded="false" aria-controls="board-date-menu"><span id="board-date-label">Today</span><span aria-hidden="true">▾</span></button><button id="board-date-next" class="board-date-arrow" type="button" aria-label="Next challenge">›</button><div id="board-date-menu" role="group" aria-label="Choose a challenge date" hidden></div></div><p id="board-description" class="board-description"></p><div id="standing-summary"><p id="your-rank" role="status"></p><p id="standing-note"></p></div>${form?`<details id="score-entry" class="score-entry" ${locked?'':'open'}><summary id="player-summary">${S.decodeName(S.encodeName(initials,badge))} · Change player</summary>${form}</details>`:''}<p id="leaderboard-status" role="status">Loading scores…</p><ol id="leaderboard-list" class="leaderboard-list" tabindex="0" aria-label="Top scores. Scroll for more."></ol><div id="your-standing" hidden aria-label="Your standing outside the displayed leaders"></div><div class="board-sync"><p id="board-updated" role="status"></p><button id="refresh-scores" class="text-button" type="button">Refresh</button></div></div>`,
    ...
    if($('score-initials')){
      ...
      $('toggle-player-lock').onclick=()=>{
        ...
        if($('player-fields').disabled){
          ...
          $('player-summary').textContent=`${S.decodeName(S.encodeName(playerProfile.initials,playerProfile.badge))} · Change player`;$('score-entry').open=false;$('dialog-details').scrollTop=0;syncPlayerEditor();$('player-summary').focus({preventScroll:true});loadLeaderboard();
    ...
    document.querySelectorAll('[data-board]').forEach(button=>{
      ...
      button.addEventListener('keydown',event=>{
        if(['ArrowLeft','ArrowRight','Home','End'].includes(event.key)){
          event.preventDefault();const next=event.key==='Home'?'daily':event.key==='End'?'alltime':boardView==='daily'?'alltime':'daily';
          selectBoard(next);$('board-tab-'+next).focus();
        }
      });
    });
    $('board-date-toggle').addEventListener('click',()=>{
      const menu=$('board-date-menu');menu.hidden=!menu.hidden;
      $('board-date-toggle').setAttribute('aria-expanded',String(!menu.hidden));
    });
    $('board-date-menu').addEventListener('click',event=>{
      const button=event.target.closest?.('[data-date-choice]');
      if(button)chooseBoardDate(button.dataset.dateChoice);
    });
    $('board-date-prev').addEventListener('click',()=>stepBoardDate(1));
    $('board-date-next').addEventListener('click',()=>stepBoardDate(-1));
    $('board-date-control').addEventListener('keydown',event=>{
      if(event.key==='Escape'&&!$('board-date-menu').hidden){event.preventDefault();event.stopPropagation();closeBoardDateMenu();$('board-date-toggle').focus();}
    });
    ...
    loadLeaderboard();boardPoll=setInterval(()=>{if(!document.hidden&&!sideways&&!$('score-entry')?.open)loadLeaderboard();},S.POLL_INTERVAL_MS);
  }
  function syncPlayerEditor(){
    const editing=!!$('score-entry')?.open,wasPlayerSummary=document.activeElement===$('player-summary');
    ...
    $('dialog-title').textContent=editing?'Your player':'Leaderboards';
    $('story-dialog').classList.toggle('player-editing',editing);
    if(editing&&wasPlayerSummary)$('score-form').querySelector('[data-letter]')?.focus({preventScroll:true});
    $('dialog-body').scrollTop=0;scheduleFit();
  }
  function stopBoardRefresh(){if(boardPoll!==null){clearInterval(boardPoll);boardPoll=null;}boardPending='';}
  function closeBoardDateMenu(){
    const menu=$('board-date-menu');if(!menu)return;
    menu.hidden=true;$('board-date-toggle').setAttribute('aria-expanded','false');
  }
  function chooseBoardDate(value){
    if(value!=='records'&&!boardDates.includes(value))return;
    boardScope=value==='records'?'records':'standings';boardDate=value==='records'?'':value;
    closeBoardDateMenu();loadLeaderboard();
  }
  function stepBoardDate(direction){
    if(boardScope==='records')return;
    const index=boardDates.indexOf(boardDate||boardDates[0]);
    const next=boardDates[index+direction];if(next)chooseBoardDate(next);
  }
  function selectBoard(view){
    boardView=view;
    closeBoardDateMenu();
    document.querySelectorAll('[data-board]').forEach(b=>{b.setAttribute('aria-selected',String(b.dataset.board===view));b.tabIndex=b.dataset.board===view?0:-1;});
    $('board-panel').setAttribute('aria-labelledby','board-tab-'+view);loadLeaderboard();
  }
  function boardPlayer(){try{return S.encodeName(playerProfile.initials,playerProfile.badge);}catch{return '';}}
  function standingRow(entry,separate=false){
    const row=document.createElement(separate?'div':'li'),rank=document.createElement('span'),name=document.createElement('strong'),score=document.createElement('span'),herd=document.createElement('small');
    row.className='standing-row'+(entry.isYou?' is-you':'');rank.className='rank';rank.textContent=String(entry.rank);
    name.textContent=S.decodeName(entry.playerName)+(entry.isYou?' · You':'');score.textContent=entry.score.toLocaleString();
    const animal=S.ANIMALS.includes(entry.biggestHerdAnimal)?entry.biggestHerdAnimal:'🐑';
    herd.textContent=`${boardView==='daily'&&boardScope==='records'&&entry.challengeDate?D.label(entry.challengeDate,true)+' · ':''}Biggest herd ${Math.max(0,Math.min(36,Number(entry.biggestHerdCount)||0))} ${animal}`;
    if(entry.winnerDate||boardView==='alltime'&&entry.dailyWins){
      const badge=document.createElement('span');badge.className='winner-badge';badge.textContent=entry.winnerDate?`♛ Daily winner · ${D.label(entry.winnerDate)}`:`♛ ${entry.dailyWins} daily ${entry.dailyWins===1?'win':'wins'}`;name.append(badge);
    }else if(entry.leading){const lead=document.createElement('span');lead.className='leading-label';lead.textContent='Leading';name.append(lead);}
    row.append(rank,name,score,herd);return row;
  }
  function renderStandings(data){
    const list=$('leaderboard-list'),scroll=list.scrollTop;list.replaceChildren();
    data.entries.forEach(entry=>list.append(standingRow(entry)));
    list.style.setProperty('--standings-height',`${48*Math.min(5,Math.max(1,data.entries.length))}px`);
    list.classList.toggle('short-list',data.entries.length<=5);list.scrollTop=scroll;
    $('board-date-control').hidden=boardView!=='daily';
    boardDates=[...new Set([data.today,...(data.dates||[]),boardDate].filter(day=>D.validDay(day)&&day<=data.today))].sort().reverse();
    const selected=boardDate||data.today;
    $('board-date-label').textContent=boardScope==='records'?'Daily records · all dates':selected===data.today?`Today · ${D.label(data.today)}`:D.label(selected,true);
    $('board-date-prev').disabled=boardScope==='records'||boardDates.indexOf(selected)>=boardDates.length-1;
    $('board-date-next').disabled=boardScope==='records'||boardDates.indexOf(selected)<=0;
    const menu=$('board-date-menu');menu.replaceChildren();
    for(const [value,label] of [[data.today,`Today · ${D.label(data.today)}`],...boardDates.filter(day=>day!==data.today).map(day=>[day,D.label(day,true)]),['records','Daily records · all dates']]){
      const button=document.createElement('button');button.type='button';button.dataset.dateChoice=value;button.textContent=label;
      button.setAttribute('aria-pressed',String(value===(boardScope==='records'?'records':selected)));menu.append(button);
    }
    $('board-description').textContent=boardView==='alltime'?'Top 20 adventures · original scores and tie rules.':boardScope==='records'?'Best daily score per player per date · late posts included.':
      data.status==='finalized'?`Final results · ${D.label(data.date,true)}`:data.date<data.today?`Closed ${D.label(D.shiftDay(data.date,1))} at midnight Eastern · official winner not verified.`:`Closes ${D.label(D.shiftDay(data.date,1))} at 12:00 a.m. Eastern.`;
    const own=data.yourStanding,daily=boardView==='daily'&&boardScope==='standings',label=daily?(data.status==='finalized'?'Your final rank':'Your current rank'):boardView==='alltime'?'Your best all-time rank':'Your best daily record';
    $('your-rank').textContent=own?`${label}: #${own.rank} of ${daily?data.participantCount:data.total} ${daily?(data.participantCount===1?'player':'players'):boardView==='alltime'?(data.total===1?'adventure':'adventures'):(data.total===1?'record':'records')}`:boardPlayer()?(daily&&data.status==='finalized'?'No eligible score for you in this challenge.':'You are not ranked here yet.'):'Post a score to see your standing.';
    $('standing-note').textContent=daily?(data.status==='finalized'?'This challenge is complete.':data.date===data.today?'Standings change as more people play.':data.authoritative?'Awaiting official finalization.':'Past standings are provisional; official winner unavailable.'):(data.authoritative?'Daily-win badges recognize finalized victories.':'Official daily-win badges are unavailable for now.');
    const separate=$('your-standing');separate.replaceChildren();separate.hidden=!own||data.entries.some(e=>e.isYou);
    if(!separate.hidden){const label=document.createElement('b');label.textContent='Your standing';separate.append(label,standingRow(own,true));}
    $('leaderboard-status').hidden=!!data.entries.length;$('leaderboard-status').textContent=data.entries.length?'':'No eligible scores yet.';
    const checked=new Date(data.updatedAt).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'});
    $('board-updated').textContent=`Updated ${checked}${data.authoritative?'':' · official winner status unavailable'}`;
    $('board-updated').classList.toggle('out-of-date',false);scheduleFit();
  }
  async function loadLeaderboard(){
    if(dialogView!=='leaderboard'||sideways)return;
    const player=boardPlayer(),key=[boardView,boardDate,boardScope,player].join('|');
    if(boardPending===key)return;
    const ticket=dialogGeneration,request=++leaderboardRequest,view=boardView,date=boardDate,scope=boardScope;
    boardPending=key;
    if(boardKey!==key){
      boardKey=key;$('leaderboard-list').scrollTop=0;
      if(boardCache.has(key))renderStandings(boardCache.get(key));
      else{$('leaderboard-list').replaceChildren();$('your-standing').hidden=true;$('your-rank').textContent='';$('standing-note').textContent='';$('board-updated').textContent='';$('board-description').textContent=view==='alltime'?'All-time adventures':'Daily Challenge';}
    }
    $('board-date-control').hidden=view!=='daily';$('leaderboard-status').hidden=false;$('leaderboard-status').textContent='Refreshing standings…';$('refresh-scores').disabled=true;
    try{
      const data=await S.consolidatedBoard(view,date,player,scope);if(ticket!==dialogGeneration||request!==leaderboardRequest)return;
      if(data.authoritative)serverOffset=data.serverTime-Date.now();
      boardCache.set(key,data);renderStandings(data);
      if(finalResult&&!submission.done){
        const eligible=!!finalResult.challengeDate||view!=='alltime'||S.qualifies(data.entries,finalResult.score);
        $('submit-score').disabled=submission.pending||!eligible;
        $('submit-status').textContent=eligible?submission.message||(!finalResult.challengeDate&&view==='daily'?'Your free-play score posts to All-Time.':''):`You need more than ${data.entries[S.LIMIT-1].score.toLocaleString()} points to enter the top 20.`;
      }
      if(finalResult&&submission.done&&submission.playerName&&view===(finalResult.challengeDate?'daily':'alltime')&&(view!=='daily'||scope==='standings'&&data.date===finalResult.challengeDate)){
        const candidates=data.entries.slice();if(data.yourStanding&&!candidates.some(e=>e.rank===data.yourStanding.rank))candidates.push(data.yourStanding);
        const rank=S.verifiedRank(candidates,finalResult,submission.playerName);
        if(rank&&rank!==finalResult.rank){
          finalResult=Object.freeze({...finalResult,rank,rankBoard:view,playerLabel:S.decodeName(submission.playerName)});
          submission.message=view==='daily'?`Your current daily rank: #${rank} of ${data.participantCount} players.`:`High score saved! You placed #${rank} on the all-time board.`;saveDaily();$('submit-status').textContent=submission.message;
          if(shareCard)URL.revokeObjectURL(shareCard.url);shareCard=null;shareReady=false;prepareShare(finalResult);
        }
      }
    }catch(error){if(ticket===dialogGeneration&&request===leaderboardRequest){
      const last=boardCache.get(key);$('leaderboard-status').hidden=false;$('leaderboard-status').textContent=last?'Refresh failed. Showing the last valid standings.':error.message||'Could not connect to the score sheet. Try Refresh standings.';
      $('board-updated').textContent=last?`Out of date · last updated ${new Date(last.updatedAt).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'})}`:'No standings loaded.';$('board-updated').classList.toggle('out-of-date',true);
    }}finally{if(ticket===dialogGeneration&&request===leaderboardRequest){boardPending='';$('refresh-scores').disabled=false;scheduleFit();}}
  }
  async function postScore(event){
    ...
    try{
      ...
      if(dialogView==='leaderboard'){$('submit-score').hidden=true;$('toggle-player-lock').hidden=true;$('score-entry')?.remove();syncPlayerEditor();$('submit-status').textContent=submission.message;boardDate=result.challengeDate===today()?'':result.challengeDate||'';boardScope='standings';boardPending='';selectBoard(result.challengeDate?'daily':'alltime');}
  ...
  document.addEventListener('visibilitychange',()=>{if(!document.hidden&&dialogView==='leaderboard')loadLeaderboard();});
  window.addEventListener('focus',()=>{if(!document.hidden&&dialogView==='leaderboard')loadLeaderboard();});
  window.addEventListener('pageshow',event=>{if(event.persisted){if(challengeDate)dailyCurrent();if(['intro','chooser'].includes(dialogView))showGameChooser(dialogView==='intro',chooserBack);}if(dialogView==='leaderboard')loadLeaderboard();scheduleFit();},{passive:true});
