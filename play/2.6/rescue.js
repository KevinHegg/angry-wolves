(() => {
  ...
  let fieldEntryBoard = null,fieldEntryCatTurns=0;
  ...
  function render() {
    ...
    $('board').innerHTML = state.board.map((t,i)=>`<button class="animal ${t===R.CAT?'magic-cat':''} ${playable.has(i)?'hint':''}" data-cell="${i}" data-type="${t}" aria-label="${t===R.CAT?`Magic cat, vanishes after ${state.catTurns} ${state.catTurns===1?'rescue':'rescues'}. Pip can scare it away`:singular[t]}, row ${Math.floor(i/6)+1}, column ${i%6+1}" aria-pressed="false" tabindex="${i===30?0:-1}">${icon(t===R.CAT?5:t)}${t===R.CAT?`<span class="cat-life" aria-hidden="true">${state.catTurns} ✦</span>`:''}</button>`).join('');
    $('bark').disabled = !state.bark || busy || state.status!=='playing';
    $('bark').innerHTML = `<span aria-hidden="true">🐕</span><span>${state.bark?'Pip, bark!':'Good dog, Pip.'}<small>${state.bark?(state.catTurns?'+3 steps · shoo cat':'+3 steps · regroup · once'):'Bark used this field'}</small></span>`;
  ...
  function select(i) {
    ...
    if(state.board[i]===R.CAT){selected=[];renderSelection();say(`A magic cat! Gone in ${state.catTurns} ${state.catTurns===1?'rescue':'rescues'}. Pip can shoo it away now.`);return;}
    ...
    const step=R.wolfStep(g.length)-R.pressure(state.moves+1);
  ...
  function commit() {
    ...
    if(state.catTurns===1)$('board').children[state.board.indexOf(R.CAT)]?.classList.add('cat-vanishing');
    ...
    setTimeout(()=>{
      ...
      if(result.catRemoved>=0)$('board').children[result.catRemoved].classList.add('new-arrival');
      ...
      say(`${result.count} ${names[result.type]} home! ${result.step>0?'The pack backs away.':result.step===0?'The pack holds back.':`The pack moves ${-result.step} ${result.step===-1?'step':'steps'} closer.`}${result.catAppeared>=0?` Poof! A cat appeared for ${state.catTurns} ${state.catTurns===1?'rescue':'rescues'}.`:result.catRemoved>=0?' Poof! The cat vanished. The gap filled.':''}${result.regrouped?' A new herd gathered.':''}`);
      if(state.status!=='playing')finishField();
    },reducedMotion.matches?0:230);
  }
  function startField(chapter, carriedBoard = null,catTurns=0) {
    generation++;busy=false;selected=[];ready=true;
    state=R.create(chapter,Math.random,carriedBoard,catTurns);fieldEntryBoard=state.board.slice();fieldEntryCatTurns=state.catTurns;render();window.scrollTo(0,0);
  ...
  function finishField() {
    if(state.status==='lost'){
      openDialog({eyebrow:'PIP GUIDED THE HERDS TO COVER',title:'A little too close.',copy:'The wolves reached the field. The animals are sheltering with Pip. Try again: three animals bring the pack two steps closer, while five or more hold them off.',
        ...
        action:'Try this field again',run:()=>{closeDialog();startField(state.chapter,fieldEntryBoard,fieldEntryCatTurns);} });return;
    }
    tone('win');
    if(state.chapter===2){
    ...
    openDialog({eyebrow:`FIELD ${state.chapter+1} OF 3 COMPLETE`,title:['The sheep are safe.','Out of the orchard.'][state.chapter],copy:`${R.CHAPTERS[state.chapter].ending} ${next.story}`,
      ...
      action:`Continue to field ${state.chapter+2}`,run:()=>{
        ...
        const chapter=state.chapter+1,board=state.board.slice(),catTurns=state.catTurns;closeDialog();tone('gate');startField(chapter,board,catTurns);
      }});
  }
  ...
  $('bark').addEventListener('click',()=>{
    ...
    const previousDistance=state.distance,catIndex=state.board.indexOf(R.CAT),ticket=generation;
    tone('bark');
    const finish=()=>{
      if(ticket!==generation)return;
      ...
      if(catIndex>=0)$('board').children[catIndex].classList.add('new-arrival');
      say(catIndex>=0?'Woof! The cat vanished. Animals tumble into the gap.':'Good dog! Wolves back 3 steps. The animals regrouped.');
    };
    if(catIndex>=0){busy=true;selected=[];renderSelection();$('bark').disabled=true;$('board').children[catIndex].classList.add('cat-vanishing');setTimeout(finish,reducedMotion.matches?0:230);}else finish();
  });
  $('retry').addEventListener('click',()=>{
    if(busy)return;
    openDialog({eyebrow:'A FRESH START',title:'Try this field again?',copy:'Earlier fields stay complete. This field’s animals, points, and wolf distance reset. Pip gets another bark, but any bark already used still counts against the rest bonus.',action:'Restart this field',run:()=>{closeDialog();startField(state.chapter,fieldEntryBoard,fieldEntryCatTurns);},secondary:'Keep playing',secondaryRun:closeDialog});
  });
