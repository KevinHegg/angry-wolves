...
(function (root) {
  ...
  const SIZE = 6;
  const CAT = 4;
  const CAT_SETTINGS = Object.freeze({enabled:true,maxCats:3,firstChapter:1,chance:.5,minMoves:1,minLife:2,maxLife:4,cooldown:1});
  ...
  function ensureMove(board, types, rng) {
    ...
    const first = [30,33,24,27,18,21,12,15,6,9,0,3].find(i=>![board[i],board[i+1],board[i+2]].includes(CAT));
    board[first] = board[first+1] = board[first+2] = t;
    return true;
  }
  function create(chapter = 0, rng = Math.random, carriedBoard = null, carriedCats = {}) {
    ...
    return { chapter, board, cats:Object.fromEntries(board.flatMap((t,i)=>t===CAT?[[i,Math.max(1,carriedCats[i]||3)]]:[])),catCooldown:0, saved: [0, 0, 0, 0], distance: c.distance, bark: 1, moves: 0, score: 0, biggest: {count: 0, type: 0}, status: 'playing' };
  }
  function rescue(state, start, rng = Math.random) {
    ...
    if(preview.cats.length)state.catCooldown=CAT_SETTINGS.cooldown;
    ...
    let catAppeared=-1;
    if(Object.keys(state.cats).length<CAT_SETTINGS.maxCats){
      if(state.catCooldown>0)state.catCooldown--;
        ...
        state.cats[catAppeared]=CAT_SETTINGS.minLife+Math.floor(rng()*(CAT_SETTINGS.maxLife-CAT_SETTINGS.minLife+1));
      }
    }
    const regrouped = ensureMove(state.board, CHAPTERS[state.chapter].types, rng);
    return { ok: true, count, type, step, cleared: g, regrouped,burstCats:preview.cats,burstAnimals,catAppeared };
  }
  function burstPreview(state){
    const cats=Object.keys(state.cats).map(Number).filter(i=>state.cats[i]===1);
    const animals=[...new Set(cats.flatMap(neighbors))].filter(i=>state.board[i]!==CAT);
    return {cats,animals};
  }
  function collapse(state,cleared,rng){
    ...
    for (let col = 0; col < SIZE; col++) {
      ...
      for (let row = SIZE - 1; row >= 0; row--){
        ...
        if(tile?.type===CAT)nextCats[index]=tile.turns;
      }
    }
    state.cats=nextCats;
  ...
  const api = { SIZE, CAT, CAT_SETTINGS, CHAPTERS, REST_BONUSES, restBonus, wolfStep, herdPoints, pressure, neighbors, group, groups, create, rescue, bark, burstPreview };
