(() => {
  ...
  function icon(type) {
    if(type===5)return '<svg viewBox="0 0 64 64" aria-hidden="true" focusable="false"><path d="M8 30L5 3L25 16Q32 12 39 16L59 3L56 32Q61 51 44 57L32 61L20 57Q3 51 8 30Z" fill="#101719" stroke="#657477" stroke-width="2"/><path d="M12 13L15 26L23 21M52 13L49 26L41 21" fill="#574c62"/><path d="M13 32Q23 25 29 34Q20 43 13 32M35 34Q43 25 52 32Q44 43 35 34" fill="#def26a"/><path d="M22 30V37M43 30V37" stroke="#0b1416" stroke-width="3"/><path d="M29 41L35 41L32 45Z" fill="#b87f9a"/><path d="M20 46Q32 58 45 45Q33 51 20 46" fill="#f7efd8"/><path d="M17 43L2 39M17 47L1 48M47 43L62 39M47 47L63 48" stroke="#c4c0b0" stroke-width="1.2"/></svg>';
  ...
  function render() {
    ...
    $('cat-warning').textContent=imminent.cats.length?`${imminent.cats.length} ${imminent.cats.length===1?'cat bursts':'cats burst'} next · +${imminent.cats.length} wolf ${imminent.cats.length===1?'step':'steps'}${state.bark?' · Pip stops them!':''}`:`${catCount} ${catCount===1?'magic cat':'magic cats'} · each number counts rescues`;
    const blastArea=new Set(imminent.animals);
    const playable = new Set(R.groups(state.board).flat());
    $('board').innerHTML = state.board.map((t,i)=>`<button class="animal ${blastArea.has(i)?'blast-warning':''} ${t===R.CAT?'magic-cat':''} ${playable.has(i)?'hint':''}" data-cell="${i}" data-type="${t}" aria-label="${t===R.CAT?`Magic cat, bursts after ${state.cats[i]} ${state.cats[i]===1?'rescue':'rescues'}. Pip can scare it away`:singular[t]}, row ${Math.floor(i/6)+1}, column ${i%6+1}" aria-pressed="false" tabindex="${i===30?0:-1}">${icon(t===R.CAT?5:t)}${t===R.CAT?`<span class="cat-life" aria-hidden="true">${state.cats[i]} ✦</span>`:''}</button>`).join('');
  ...
  function select(i) {
    ...
    if(state.board[i]===R.CAT){selected=[];renderSelection();say(`A magic cat! Bursts in ${state.cats[i]} ${state.cats[i]===1?'rescue':'rescues'}. Pip can quietly shoo all cats away.`);return;}
  ...
  function commit() {
    ...
    setTimeout(()=>{
      ...
      say(`${result.count} ${names[result.type]} home! ${result.step>0?'The pack backs away.':result.step===0?'The pack holds back.':`The pack moves ${-result.step} ${result.step===-1?'step':'steps'} closer.`}${result.catAppeared>=0?` Poof! A cat appeared for ${state.cats[result.catAppeared]} rescues.`:result.burstCats.length?` Poof! ${result.burstCats.length===1?'A cat burst':`${result.burstCats.length} cat bursts`} rescued ${result.burstAnimals.length} extra animals, no extra points.`:''}${result.regrouped?' A new herd gathered.':''}`);
  ...
  function finishField() {
    ...
    openDialog({eyebrow:`FIELD ${state.chapter+1} OF 3 COMPLETE`,title:['The sheep are safe.','Out of the orchard.'][state.chapter],copy:`${R.CHAPTERS[state.chapter].ending} ${next.story}`,
      details:`<div class="stat-line"><span><strong>${total+state.saved.reduce((a,b)=>a+b,0)}</strong>animals home</span><span><strong>${rested}</strong>fields Pip rested</span></div><p class="dialog-tip">Next: ${next.name}. Your animals carry forward. Pip gets a fresh bark. From field two, up to three black cats may appear with 2–4 rescue timers. Bursts rescue neighbors but attract wolves. Pip can dismiss all cats quietly.</p>`,
