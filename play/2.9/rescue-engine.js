...
(function (root) {
  ...
  function rescue(state, start, rng = Math.random) {
    ...
    for(const i of Object.keys(state.cats).map(Number))if(state.board[i]===GOOD_CAT && state.cats[i]<=0){
      const gift=bestGift(state,i);gift.cells.forEach(n=>state.board[n]=gift.type);delete state.cats[i];gifts.push(gift);
    }
  ...
  // Try every animal on the actual board after gravity. Score the connected
  // herd through the cat; break ties toward unfinished goals, then animal order.
  function bestGift(state,index){
    const cells=[index,...neighbors(index).filter(i=>!isCat(state.board[i]))];
    let best=null;
    for(let type=0;type<CHAPTERS[state.chapter].types;type++){
      const board=state.board.slice();cells.forEach(i=>board[i]=type);
      const size=group(board,index).length,need=Math.max(0,CHAPTERS[state.chapter].goal[type]-state.saved[type]);
      if(!best||size>best.size||(size===best.size&&need>best.need))best={cells,type,size,need};
    }
    return best;
  }
  ...
  const api = { SIZE, CAT, GOOD_CAT, isCat, bestGift, CAT_SETTINGS, CHAPTERS, REST_BONUSES, restBonus, wolfStep, herdPoints, pressure, neighbors, group, groups, create, rescue, bark, burstPreview };
