(() => {
  ...
  function select(i) {
    ...
    if(R.isCat(state.board[i])){selected=[];renderSelection();say(`${state.board[i]===R.GOOD_CAT?'A smiling cat! Makes the largest possible herd in':'An evil cat! Bursts in'} ${state.cats[i]} ${state.cats[i]===1?'rescue':'rescues'}. Pip can quietly shoo all cats away.`);return;}
  ...
  function commit() {
    ...
    setTimeout(()=>{
      ...
      result.gifts.forEach(gift=>gift.cells.forEach(i=>$('board').children[i].classList.add('gift-arrival')));
