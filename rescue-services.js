...
(function(root) {
  ...
  const BADGE_NAMES = Object.freeze(['Sheepdog', 'Sheep', 'Ram', 'Goat', 'Faithful hound', 'Walking boot', 'Wheat', 'Basket', 'Farmhouse', 'Sunrise']);
  ...
  function decodeName(value) {
    const match = /^([A-Z]{3})([0-9])$/.exec(String(value));
    return match ? `${match[1]} ${BADGES[Number(match[2])]}` : '??? 🐑';
  }
  ...
  async function leaderboard() {
    const data = await request(`${URL}?gameMode=${MODE}&limit=20`, {cache:'no-store'});
    if (!data.ok || !Array.isArray(data.entries)) throw new Error('Could not load the shepherds’ board.');
    return data.entries.filter(e => e.gameMode === MODE && /^[A-Z]{3}[0-9]$/.test(e.playerName) && Number.isFinite(e.score) && e.score >= 0)
