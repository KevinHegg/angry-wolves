...
for(let seed=1;seed<=2000;seed++){
 let n=seed;const rng=()=>((n=(n*1664525+1013904223)>>>0)/4294967296);let board=null,cats={},wait=null,distance=R.CHAPTERS[0].distance;
 for(let chapter=0;chapter<3;chapter++){
  const s=R.create(chapter,rng,board,cats,undefined,wait);s.distance=distance;
  ...
  if(s.status!=='won')break;board=s.board.slice();cats={...s.cats};wait=s.windWait;distance=s.distance;
 }
}
