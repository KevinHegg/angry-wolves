...
for (const policy of ['emergency bark', 'let Pip rest']) {
  ...
  for(let seed=1;seed<=trials;seed++){
    const rng=seeded(seed);let score=0,rests=0,won=true,carry=null,cats={},distance=R.CHAPTERS[0].distance,wait=null;
    for(let chapter=0;chapter<3;chapter++){
      const state=R.create(chapter,rng,carry,cats,undefined,wait);state.distance=distance;
      while(state.status==='playing'&&state.moves<100){
      ...
      carry=state.board.slice();cats={...state.cats};distance=state.distance;wait=state.windWait;score+=state.score;rests+=state.bark;
    }
