...
function harness(reducedMotion=true,storage=memoryStorage(),timers=null,services={},initialLandscape=false,trailGeometry=false,gameDate=Date){
 ...
 vm.runInNewContext(source,{window,document,navigator:{clipboard:{writeText:async text=>{copied.push(text);}}},localStorage:storage,ResizeObserver:class{observe(){}},requestAnimationFrame:()=>1,setInterval:fn=>{intervals.set(++intervalId,fn);return intervalId;},clearInterval:id=>intervals.delete(id),setTimeout:(fn,delay)=>{if(!timers)return fn();const timer={fn,delay};timers.push(timer);return timer;},clearTimeout:timer=>{if(timer)timer.cancelled=true;},URL:{revokeObjectURL(){}},performance:{now:()=>clock.now},Math,Date:gameDate});
 return{...window.test,api:window.test,nodes,get,posted,copied,shareTools,animations,sounds,badgeInput,clock,document,windowEvents,documentEvents,intervals,rotate:landscape=>{orientation.matches=landscape;orientation.changed();}};
}
const settle=()=>new Promise(resolve=>setImmediate(resolve));
test('results copy the full caption with the link and reveal image tools only without file sharing',async()=>{
 const h=harness(),a=h.api;a.start();a.state.chapter=2;a.state.status='won';a.finishField();await settle();
 assert.equal(a.view,'results');assert.equal(h.get('image-tools').hidden,true);
 h.get('copy-caption').events.click();await settle();
 assert.equal(h.copied[0],S.caption(a.result));assert.ok(h.copied[0].endsWith(S.GAME_URL));assert.equal(h.get('share-status').textContent,'Score and link copied.');
 h.shareTools.files=false;h.shareTools.copy=true;a.showResults();await settle();
 assert.equal(h.get('image-tools').hidden,false);assert.equal(h.get('copy-image').hidden,false);
 h.get('copy-image').events.click();await settle();assert.equal(h.get('share-status').textContent,'Score image copied.');
 h.get('save-image').events.click();assert.equal(h.get('share-status').textContent,'Your score image is downloading.');
 h.shareTools.copy=false;a.showResults();await settle();assert.equal(h.get('copy-image').hidden,true);
});
